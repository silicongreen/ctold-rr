...
class CoursesController < ApplicationController
  ...
  def manage_batches
    ...
    if @batches.length == 1
        ...
        @courses = Rails.cache.fetch("course_data_#{batch_name.parameterize("_")}_#{school_id}"){
          ...
          @courses = Course.find(:all, :conditions => ["courses.id IN (?) and courses.is_deleted = 0 and batches.name = ?", @batch_ids, batch_name], :select => "courses.*,  GROUP_CONCAT(courses.section_name,'-',courses.id,'-',batches.id) as courses_batches", :joins=> "INNER JOIN `batches` ON batches.course_id = courses.id", :group => 'course_name', :order => "cast(replace(course_name, 'Class ', '') as SIGNED INTEGER) asc")
          @tmp_courses 
        }
    end
  end
  ...
  def get_batches_classes
    ...
    @courses = Rails.cache.fetch("course_data_#{batch_name.parameterize("_")}_#{school_id}"){
      ...
      @tmp_courses = Course.find(:all, :conditions => ["courses.id IN (?) and courses.is_deleted = 0 and batches.name = ?", @batch_ids, batch_name], :select => "courses.*,  GROUP_CONCAT(courses.section_name,'-',courses.id,'-',batches.id) as courses_batches", :joins=> "INNER JOIN `batches` ON batches.course_id = courses.id", :group => 'course_name', :order => "course_name asc")
      @tmp_courses
    }
