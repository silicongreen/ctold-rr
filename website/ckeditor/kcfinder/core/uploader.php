...
//
    ...
    public function resizeforfacebook(  $str_path =  '/var/www/dsdata/ckeditor/kcfinder/core/../../../upload/gallery/')
    {
        $real_path = "/var/www/dsdata/upload/gallery";
        print str_replace('uploader.php', '', __FILE__);
        $dirIterator = opendir( $str_path );
        while($strDir = readdir($dirIterator))
        {
            print $strDir . "<Br />";
            if(is_dir($str_path . $strDir . DIRECTORY_SEPARATOR) && $strDir != '.' && $strDir != '..')
            {
                $this->resizeforfacebook($str_path . $strDir . DIRECTORY_SEPARATOR, $objConn);
            }
            else
            {
                if ( $strDir != "." && $strDir != ".." )
                {
                    $ar_image_info = pathinfo( $str_path . $strDir);
                    $file = $str_path . $strDir;
                    print_r($ar_image_info);
                    
                    echo "filename: " . $str_path . " $strDir\n<br />";
                }
            }
        }
    }
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        $mainFile = $file;
        $i=0;
        foreach($this->config['thumbArray'] as $key=>$value)
        {
            ...
            $thumb = substr($mainFile, strlen($this->config['uploadDir']));           
            $thumb = $this->config['uploadDir'] . "/" . $key . $thumb;
            ...
            if (!is_dir($thumbDir))
            {
                @mkdir($thumbDir, $this->config['dirPerms'], true);
            }
            ...
            if($value[1] == 0)
            {
               $value[1] =  $value[0]*($gd->get_height()/$gd->get_width());
            }    

            // Images with smaller resolutions than thumbnails
            if (($gd->get_width() <= $value[0]) &&
                ($gd->get_height() <= $value[1])
            ) {
            ...
            elseif($key == "facebook")
            {
                if(!$gd->resize($value[0], $value[1]))
                {
                   return false; 
                }
                else
                {
                    $gd->resize_crop($mainFile);
                     
                }    
            }
            else if($key == "mobile" || $key == "mobile1" || $key == "mobile2" || $key == "mobile3")
            {
                if(!$gd->resize($value[0], $value[1]))
                {
                   return false; 
         ...
         /*   elseif($key == "otherSixBottom")
            ...
            elseif(!$gd->resize_fit($value[0], $value[1]))
            {
            ...
            if($key=="thumbs")
            {
                $mainThumb = $gd->imagejpeg($thumb, $this->config['jpegQuality']);
            }
            else
            {
