...
class browser extends uploader {
    ...
    protected function act_init() {
        ...
        foreach ($tree['dirs'] as $t)
        {
            if (isset($t['dirs']))
            {
                //$this->current_dir = $this->typeDir . 
...
//        
    ...
    protected function act_showvideo() 
    {
        ...
        $hRes = mysqli_iquery($strSQL);
        if ( mysqli_num_rows($hRes) > 0 )
        {
            return json_encode(mysql_fetch_object($hRes));
        }
        else 
        {
            return false;
    ...
    protected function save_material( $s_target, $dir )
    {
        ...
        $strSQL = "INSERT INTO " . $db_config['prefix'] . "materials SET `material_url` = '" . $s_target . "', 
