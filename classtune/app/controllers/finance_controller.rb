...
#        @transactions_particular = FinanceTransactionParticular.find(:all, :order => 'finance_transactions.transaction_date desc', :conditions => ["finance_transactions.transaction_date >= '#{@start_date}' and finance_transactions.transaction_date <= '#{@end_date}'"],:include=>"finance_transaction")
  ...
  def fee_collection_batch_list
    if params[:id].present?
      ...
      @batches = Batch.active.find(:all,:joins=>[{:finance_fee_particulars=>:finance_fee_category},:course],:conditions=>"finance_fee_categories.id =#{@fee_category.id} and finance_fee_particulars.is_deleted=#{false}",:order=>"courses.code ASC").uniq
      @fee_particulars = FinanceFeeParticular.find(:all, :conditions=>"finance_fee_category_id=#{@fee_category.id} and is_tmp=#{true}",:group=>"finance_fee_particular_category_id").uniq
  ...
  def fee_collection_create
    ...
    finance_fee_category_id = params[:finance_fee_collection][:fee_category_id]
    finance_fee_category = FinanceFeeCategory.find(finance_fee_category_id)
    new_finance_fee_category = FinanceFeeCategory.new
    new_finance_fee_category.name = params[:finance_fee_collection][:name]
    ...
    particular_ids.each do |p_id|
      finance_fee_particular = FinanceFeeParticular.find(:first, :conditions => "id = #{p_id.to_i}")
      unless finance_fee_particular.nil?
        unless finance_fee_particular.nil?
          fee_particular_category_id = finance_fee_particular.finance_fee_particular_category_id
          if finance_fee_particular.finance_fee_category_id.to_i == finance_fee_category_id.to_i and finance_fee_particular.is_tmp  and finance_fee_particular.opt 
            finance_fee_particulars = FinanceFeeParticular.find(:all, :conditions => "finance_fee_particular_category_id = #{fee_particular_category_id} and is_tmp = #{true} and opt = #{true}")
          else
            finance_fee_particulars = FinanceFeeParticular.find(:all, :conditions => "finance_fee_particular_category_id = #{fee_particular_category_id} and finance_fee_category_id = #{finance_fee_category_id}")
          end
          finance_fee_particulars.each do |ffp|
            p_name = ffp.name
            particular_names = params[:particular_name]
            particular_names.each do |pn|
              p_names = pn.split("_")
              p_name_id = p_names[0]
              if p_name_id.to_i == ffp.id.to_i
                p_name = pn.gsub(p_id.to_s + "_", "")
              end
            end
            ...
            if ffp.finance_fee_category_id.to_i == finance_fee_category_id.to_i and ffp.is_tmp and ffp.opt 
              tmp_finance_fee_particular = FinanceFeeParticular.find(ffp.id.to_i)
              tmp_finance_fee_particular.destroy
            end
          end
        end
      end
    ...
    params[:finance_fee_collection][:fee_category_id] = new_finance_fee_category.id
  ...
  def fee_collection_dates_batch
    if params[:id].present?
      ...
      @finance_fee_collections = @batch.finance_fee_collections.find(:all, :conditions => "is_advance_fee_collection = #{false}")
      render :update do |page|
      ...
      unless f_report.blank?
        ...
        if advance_fee_collection
          if @fee_collection_advances.particular_id == 0
            ...
            @all_fee_particulars[student.id] = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@batch.id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@batch)}
          end
        else
          @all_fee_particulars[student.id] = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@batch.id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@batch)}
        end
    ...
    if request.post?
      ...
      if advance_fee_collection
        if @fee_collection_advances.particular_id == 0
          ...
          @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@batch.id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@batch) }
        end
      else
        @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@batch.id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@batch) }
      end
...
#    end
      ...
      if @dates_array.count == 1
        ...
        if advance_fee_collection
          if @fee_collection_advances.particular_id == 0
            ...
            @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@financefee.batch_id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@financefee.batch) }
          end
        else
          @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@financefee.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@financefee.batch) }
        end
  ...
  def update_student_vat_ajax
    ...
    if advance_fee_collection
      if @fee_collection_advances.particular_id == 0
        ...
        @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@student.batch_id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@student.batch) }
      end
    else
      @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@student.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@student.batch) }
    end
  ...
  def fees_structure_for_student
    ...
    if advance_fee_collection
      if @fee_collection_advances.particular_id == 0
        @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@finance_fee.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@finance_fee.batch) }
      else
        @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@finance_fee.batch_id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@finance_fee.batch) }
      end
    else
      @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@finance_fee.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@finance_fee.batch) }
    end
  ...
  def fees_defaulters_students
    ...
    unless params[:date] == '0'
      ...
      @defaulters=Student.find(:all,:joins=>"INNER JOIN finance_fees on finance_fees.student_id=students.id and students.batch_id = finance_fees.batch_id ",:conditions=>["finance_fees.fee_collection_id='#{@date.id}' and finance_fees.balance > 0 and finance_fees.batch_id='#{@batch.id}'"],:order=>"students.first_name ASC").uniq
    else
      @date = 0
      @defaulters=Student.find(:all,:joins=>"INNER JOIN finance_fees on finance_fees.student_id=students.id and students.batch_id = finance_fees.batch_id ",:conditions=>["finance_fees.balance > 0 and finance_fees.batch_id='#{@batch.id}'"],:order=>"students.first_name ASC").uniq
    end
    render :update do |page|
  ...
  def fee_defaulters_pdf
    ...
    unless params[:date] == '0'
      ...
      @defaulters=Student.find(:all,:joins=>"INNER JOIN finance_fees on finance_fees.student_id=students.id and students.batch_id = finance_fees.batch_id",:conditions=>["finance_fees.fee_collection_id='#{@date.id}' and finance_fees.balance > 0 and finance_fees.batch_id='#{@batch.id}'"],:select=>["students.*,finance_fees.balance as balance"],:order=>"students.class_roll_no ASC").uniq
    else
      # @date = @finance_fee_collection = FinanceFeeCollection.find(params[:date])
      @date = 0
      @defaulters=Student.find(:all,:joins=>"INNER JOIN finance_fees on finance_fees.student_id=students.id and students.batch_id = finance_fees.batch_id",:conditions=>["finance_fees.balance > 0 and finance_fees.batch_id='#{@batch.id}'"],:select=>["students.*,finance_fees.balance as balance"],:order=>"students.class_roll_no ASC").uniq
    end
