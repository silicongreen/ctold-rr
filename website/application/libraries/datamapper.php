...
class DataMapper implements IteratorAggregate {
	 ...
	 */
	public function update($field, $value = NULL, $escape_values = TRUE, $where_id = NULL, $where_value = "")
	{
		if( ! is_array($field))
		{
		...
		foreach($field as $k => $v)
		{
			...
			$this->db->set($k, $v, $escape_values);
                ...
                if ( !is_null($where_id) )
                {
                    $this->db->where($where_id, $where_value);
                }
	 ...
	 */
	public function count($exclude_ids = NULL, $column = NULL, $related_id = NULL, $s_join_type = 'LEFT OUTER')
	{
		...
		if ( ! empty($this->parent))
		{
			...
			if( ! empty($this->db->ar_where))
			{
				...
				{
					$this->db->join($this->table, $this->table . '.id = ' . $relationship_table . '.' . $this_model.'_id', $s_join_type);
				}
	 ...
	 */
	public function _add_related_table($object, $related_field = '', $id_only = FALSE, $db = NULL, &$query_related = NULL, $name_prepend = '', $this_table = NULL, $s_join_type = "LEFT OUTER", $s_column_name = "")
	{
		if ( is_string($object))
		{
		...
		if ($relationship_table == $this->table && in_array($other_column, $this->fields))
		{
			...
			{
                            	$db->join($object->table . ' ' .$object_as, $object_as . '.id = ' . $this_table . '.' . $other_column, $s_join_type);
		...
		else if ($relationship_table == $object->table && in_array($this_column, $object->fields))
		{
			...
			{
                            if ( strlen($s_column_name) == 0 )
                            {
                            	$db->join($object->table . ' ' .$object_as, $this_table . '.id = ' . $object_as . '.' . $this_column, $s_join_type);
                            }
                            else
                            {
                                $db->join($object->table . ' ' .$object_as, $this_table . '.' . $s_column_name . ' = ' . $object_as . '.' . $this_column, $s_join_type);
                            }
				$query_related[] = $object_as;
			}
			if($id_only)
		...
		else
		{
			...
			if ( ! in_array($relationship_as, $query_related))
			{
                                if ( strlen($s_column_name) == 0 )
                                {
                                    $db->join($relationship_table . ' ' . $relationship_as, $this_table . '.id = ' . $relationship_as . '.' . $this_column, $s_join_type);
                                }
                                else
                                {
                                    $db->join($relationship_table . ' ' . $relationship_as, $this_table . '.' . $s_column_name . ' = ' . $relationship_as . '.' . $this_column, $s_join_type);
                                }
			...
			{
				// Add join if not already included
				$db->join($object->table . ' ' . $object_as, $object_as . '.id = ' . $relationship_as . '.' . $other_column, $s_join_type);
	 ...
	 * @return	DataMapper Returns self for method chaining.
	 */
	public function include_related($related_field, $fields = NULL, $append_name = TRUE, $instantiate = FALSE, $id_only = FALSE, $s_join_type = "LEFT OUTER" ,$s_table_related = NULL, $s_column_name = "")
	{
		if (is_object($related_field))
		{
		...
		foreach($rfs as $rf)
		{
			...
			$last =& $last->_get_without_auto_populating($rf);
                ...
                $query_related = NULL;
                $table = $this->_add_related_table($object, $related_field, $id_only, NULL, $query_related, '', $s_table_related, $s_join_type, $s_column_name);
