  ...
  def campus_report_view
    ...
    unless params[:report_type].nil? or params[:report_type].empty? or params[:report_type].blank?
      ...
      unless @report_type.to_i == 3
        ...
        @employees_all = Employee.find(:all, :conditions=>conditions + " date BETWEEN '" + @report_date_from + "' and '" + @date_today + "' and type = 1", :select => "employees.id,employees.user_id, concat( employees.employee_number, ' - ', employees.first_name )  as employee_info, employees.employee_department_id, '' as in_time, '' as out_time, '' as stat", :joins => "INNER JOIN card_attendance ON employees.user_id = card_attendance.user_id", :group => "employees.user_id")
        @employees = Employee.paginate(:conditions=>conditions + " date BETWEEN '" + @report_date_from + "' and '" + @date_today + "' and type = 1", :select => "employees.id,employees.user_id, concat( employees.employee_number, ' - ', employees.first_name )  as employee_info, employees.employee_department_id, '' as in_time, '' as out_time, '' as stat", :joins => "INNER JOIN card_attendance ON employees.user_id = card_attendance.user_id", :page => page.to_i, :per_page => per_page.to_i,:order=>""  + order_str, :group => "employees.user_id")
