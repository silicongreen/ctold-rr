...
class FinanceFeeCategories extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('batch_id, is_deleted, is_master, school_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>255),
			array('description, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, batch_id, is_deleted, is_master, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
