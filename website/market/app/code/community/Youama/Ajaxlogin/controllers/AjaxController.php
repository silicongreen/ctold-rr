...
class Youama_Ajaxlogin_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
		...
		if (isset($_POST['ajax'])){
            ...
            }else if ($_POST['ajax'] == 'logout' && Mage::helper('customer')->isLoggedIn() == true) {                
				$logout = Mage::getSingleton('youama_ajaxlogin/ajaxlogout');
                echo $logout->getResult();
            }			
        }
		if(isset($_GET['ajax']))
		{
			
			if ($_GET['ajax'] == 'logout' && Mage::helper('customer')->isLoggedIn() == true) {                
				$logout = Mage::getSingleton('youama_ajaxlogin/ajaxlogout');
                echo $logout->getResult();
            }else if ($_GET['ajax'] == 'login' && Mage::helper('customer')->isLoggedIn() != true) {
                $register = Mage::getSingleton('youama_ajaxlogin/ajaxlogin');
                echo $register->getResult();
            }
