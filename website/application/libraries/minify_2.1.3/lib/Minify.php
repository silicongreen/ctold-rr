...
class Minify {
     ...
     * "headers" (array).
     */
    public static function serve($controller, $options = array(), $type = "JS", $cache_name = "Minify")
    {
        $ci = & get_instance();
        
        if ( $content = $ci->cache->get($cache_name)  )
        {
        ...
        if (! self::$_options['quiet']) {
            ...
            if ($cacheIsReady) {
               ...
               $ci->cache->save($cache_name, $content, self::$_options['maxAge']); 
               echo $content;
            }
        } else {
            return array(
