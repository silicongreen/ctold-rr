...
class OnlineExamQuestions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('online_exam_group_id, school_id', 'numerical', 'integerOnly'=>true),
			array('mark', 'length', 'max'=>7),
			array('question, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, online_exam_group_id, question, mark, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
