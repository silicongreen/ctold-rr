 ...
 Program :"+@batch.course.course_name.to_s+" || Group :"+group_name.to_s+" || Section :"+@batch.course.section_name.to_s+" || Shift :"+batch_split[0]+" || Session :"+@batch.course.session.to_s+" || Version :"+version+"
  ...
  def render_connect_exam(template,for_save=false,file_name="")
    ...
    if MultiSchool.current_school.id == 246
      ...
      if @connect_exam_obj.result_type == 1 or @connect_exam_obj.result_type == 3 or @connect_exam_obj.result_type == 5 or @connect_exam_obj.result_type == 7 or @connect_exam_obj.result_type == 9 or @connect_exam_obj.result_type.to_i == 11 or @connect_exam_obj.result_type.to_i == 12 or @connect_exam_obj.result_type.to_i == 13 or @connect_exam_obj.result_type.to_i == 14 or @connect_exam_obj.result_type.to_i == 15
        render :pdf => template,
      ...
      if MultiSchool.current_school.id == 319  and (@connect_exam_obj.result_type == 2 or @connect_exam_obj.result_type == 3 or @connect_exam_obj.result_type == 5 or @connect_exam_obj.result_type == 7 or @connect_exam_obj.result_type == 8 or @connect_exam_obj.result_type == 9)
        render :pdf => template,
          ...
          :header => {:html => { :template=> 'layouts/pdf_empty_header.html'}},
          :footer => {:html => { :template=> 'layouts/pdf_empty_footer.html'}}
      elsif MultiSchool.current_school.id == 323 and (@connect_exam_obj.result_type == 9 or @connect_exam_obj.result_type == 10)
        render :pdf => template,
            ...
            tab['subjects'].each do |sub|
              ...
              if batch_subject_id.include?(sub['id'].to_i) or std_subject_id.include?(sub['id'].to_i)
                ...
                if @connect_exam_obj.result_type.to_i == 14
                  full_mark_subject = full_mark_subject*2
                  total_mark_subject = total_mark_subject*2
                end
                main_mark = (total_mark_subject.to_f/full_mark_subject.to_f)*100
                grade = GradingLevel.percentage_to_grade(main_mark, @batch.id)
                if !grade.blank? and !grade.name.blank? and ( grade.credit_points.to_i == 0 or total_mark_subject < 20 or (@connect_exam_obj.result_type.to_i == 14 and total_mark_subject < 40) ) and sub['subject_group_id'].to_i == 0
                  ...
                  tab['subjects'].each do |sub2|
                    ...
                    tab['exams'].each do |rs|
                      if !rs['result'].blank? and !rs['result'][rs['exam_id']].blank? and !rs['result'][rs['exam_id']][sub2['id']].blank? and !rs['result'][rs['exam_id']][sub2['id']][std['id']].blank?
                        full_mark_subject2 = full_mark_subject2+(rs['result'][rs['exam_id']][sub2['id']][std['id']]['full_mark'].to_f*2)
                        total_mark_subject2 = total_mark_subject2+(rs['result'][rs['exam_id']][sub2['id']][std['id']]['marks_obtained'].to_f*2)
                      end
                    end
                    if @connect_exam_obj.result_type.to_i == 14
                      full_mark_subject2 = full_mark_subject2*2
                      total_mark_subject2 = total_mark_subject2*2
                    end
                    ...
                    grade = GradingLevel.percentage_to_grade(main_mark, @batch.id)
                    if !grade.blank? and !grade.name.blank? and (grade.credit_points.to_i == 0 or (@connect_exam_obj.result_type.to_i == 14 and total_mark_subject < 80)) and sub['subject_group_id'].to_i > 0
                      if fourth_subject.blank?
                        u_grade = u_grade+1
                        subject_failed = true
                      else
