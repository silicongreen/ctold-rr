...
class Employee_grade extends CI_Model {

    public $table_name = 'employee_grades';
    private $_now;
    private $_school_id;
    ...
    private function preprocess($data, $mode = 'create') {
        ...
        $i = 0;
        $ar_priority = $this->getMaxPriority();
        $max_priority = (int) $ar_priority['priority'];

        $response = array();
        foreach ($data as $key => $value) {

            $response[$i]['name'] = $value;
            $response[$i]['priority'] = $max_priority + 1;
            $response[$i]['max_hours_day'] = 6;
            $response[$i]['max_hours_week'] = 40;
