...
var maxscrolltop = 1700;
var maxscrolltopIpad = 1700;
...
$(document).ready(function () {
	...
	if (windowheight < 700) {
		$('#cronWrap').css('height', '700px');
		cronheight = 700;
		if (windowheight < 750) {
			$('#cronnav').css('top', 'auto').css('bottom', '10px');
		}
		maxscrolltop = 1700 + cronheight - windowheight;
	} else {
	...
	$('#mainnav LI').click(function () {
		...
		if (hidestat === 1) {
			actscrolltop = actscrolltop + 1700;
		}
		var menuanitime = 1000;
		var menuhisanitime = 3000;
		if (actscrolltop < 1700) {
			actscrollid = 1;
		...
		if (actscrollid === 1 && (scrolltoid === 'before' || scrolltoid === 'start' || scrolltoid === 'cron')) {
			...
			$('html, body').animate({scrollTop: scrollpoints[scrolltoid]["pos"]});
			$('#cronnav').hide();
			$('#ww_button_left').hide();
			$('#ww_button_right').hide();
			$('#ww_button_down').hide();
	...
	$(document).everyTime(150, 'scrollTop', function () {
		...
		if ( (actscrolltop >= maxscrolltop && hidestat === 0) || (actscrolltop <= (maxscrolltop - 1700) && hidestat === 1) ) {
			topmode = 1;
			$('#cronnav').fadeIn(1000);
			if (ipad) {
				if (newscrollpos !== 0) {
					...
					if (newscrollpos >= 0 && newscrollpos <= cw4scroll) {
						...
						if (newscrollpos === 2000) {
							if (hidestat === 0) {
								...
								$('#beforeWrap, #startWrap').show();
								$('#cronWrap').css('top', '1700px');
								//$('#imagesWrap').css('top', (1700+cronheight)+'px');
								$(window).scrollTop(maxscrolltop);
								hidestat = 0;
							}
		...
		} else if (actmmitem !== 1 && actscrolltop > 850 && actscrolltop < 1750 && hidestat === 0) {
			$('#mainnav LI').removeClass('act');
		...
		} else if (actmmitem !== 2 && ((actscrolltop > 1750 && hidestat === 0) || (actscrolltop === 0 && hidestat === 1))) {
			$('#mainnav LI').removeClass('act');
		...
		} else if (actmmitem !== 3 && actscrolltop > 0 && actscrolltop < 1700 && hidestat === 1) {
			$('#mainnav LI').removeClass('act');
	...
	var box_atime = 1000;
	var box_ani = false;
	$('#boxesBG .box').mouseover(function () {
		$(this).find('.hover').css('left', '0px');
		$(this).find('.hover_white').css('left', '-10000px');
	}).mouseout(function () {
		$(this).find('.hover').css('left', '-10000px');
		if (box_open) {
			$(this).find('.hover_white').css('left', '0px');
		}
	})
	$('#boxesBG .box').on('click touchend', function () {
		if (!box_ani) {
			box_ani = true;
			boxid = $(this).attr('id').replace('box_', '');
			if (box_open == boxid) {
				$('#clayer_' + boxid).find('.nav').find('SPAN').first().trigger('click');
				$('.clayer').fadeOut(box_atime);
				$('#beforeheader').fadeIn(box_atime);
				$('#boxesBG').animate( {height: '-=430', top: '+=170'}, box_atime, function() {
					$('#boxesBG .hover_white').css('left', '-10000px');
				...
				var ytplayer = $('#clayer_' + box_open).find('OBJECT').find('EMBED');
				if (ytplayer.length > 0 && ytplayer.attr('allowfullscreen') === true && !ie && !ie9) {
					ytplayer.attr('id', 'yte_' + box_open);
					ytplayer = document.getElementById("yte_" + box_open);
					ytplayer.pauseVideo();
				}

				$('.clayer').fadeOut(box_atime);
				$('#boxesBG').animate( {height: '-=430', top: '+=170'}, box_atime, function() {
					$('#clayer_' + boxid).fadeIn(box_atime);
					$('#boxesBG').animate( {height: '+=430', top: '-=170'}, box_atime, function () {
						box_ani = false;
					});
				});
				...
				$('#beforeheader').fadeOut(box_atime);
				$('#clayer_' + boxid).fadeIn(box_atime);
				$('#boxesBG .hover_white').css('left', '0px');
				$('#boxesBG').animate( {height: '+=430', top: '-=170'}, box_atime, function () {
					box_ani = false;
				});
				box_open = boxid;
