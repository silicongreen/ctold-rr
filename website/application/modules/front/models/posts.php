...
//        ),
    ...
    function user_preference_tree($id=0, $parent_id=NULL)
    {
        $obj_category = new Category();
        $array = array('parent_id' => $parent_id, 'status' => 1,'show'=>1);
        ...
        if (count($obj_category) > 0)
        {
            foreach ($obj_category as $value)
            {
                $str_pref_category = $this->db->get_where('free_user_preference', array('free_user_id' => get_free_user_session('id')))->row();
                $ar_pref_category = explode(',', $str_pref_category->category_ids);
                
                $checked = (in_array($value->id, $ar_pref_category)) ? "checked='checked'" : "";
                
                $category_id = ($value->game_type)?"id='game_type'":"";
                if($parent_id!=NULL)
                $html.= "<ul>";   
                $cat_name = ( !empty($value->display_name) ) ? $value->display_name : $value->name;
                $html.= '<li><input class="user_pref_chk" type="checkbox" value="'.$value->id.'" '.$category_id.' '.$checked.' name="category[]"><span>'.$cat_name.'</span>';

                $html.= $this->user_preference_tree($id,$value->id);
                $html.= "</li>";   
                if($parent_id!=NULL)
                $html.= "</ul>";
            }
    ...
    function user_preference_tree_for_pref($id=0, $parent_id = NULL)
    {
        $obj_category = new Category();
        $array = array('parent_id' => $parent_id, 'status' => 1,'show'=>1);
        ...
        if (count($obj_category) > 0)
        {
            ...
            foreach ($obj_category as $value)
            {
                ...
                $ar_pref_category = array();
                if($str_pref_category->category_ids)
                {
                    $ar_pref_category = explode(',', $str_pref_category->category_ids);
                }
                
                if($ar_pref_category && count($ar_pref_category>0))
                {
                    $checked = (in_array($value->id, $ar_pref_category)) ? "checked='checked'" : "";
                }
                else
                ...
                $ex_margin = ($i > 0) ? 'margin-top: 18px;' : '';
                
                if(empty($parent_id)){
                    $icon_url = (!empty($value->menu_icon)) ? base_url($value->menu_icon) : base_url('styles/layouts/tdsfront/image/C.png');
                    $html .= '<div class="col-lg-12" style="border: 1px solid #ccccc9; float: left; background-color: #F1F5F6; '.$ex_margin.'">

                                <div class="col-lg-2 pref-tree-image">
                                    <img src="'.$icon_url.'" />
                                </div>

                                <div class="col-lg-10 pref-tree-list-wrap">

                                    <div class="col-lg-12 f5" style="padding: 10px 10px 5px 15px;">
                                        <input class="user_pref_chk custom-checkbox parent-chk" type="checkbox" value="'.$value->id.'" '.$category_id.' '.$checked.' name="category[]" />
                                        <label style="margin-left: 10px;">'.$cat_name.'</label>
                                    </div>';
                    ...
                    if (!empty($parent_id)){
                          $html .= '<div class="col-lg-12">';
                              $html .= '<ul>';
                                $html .= '<li><input class="user_pref_chk custom-checkbox" type="checkbox" value="'.$value->id.'" '.$category_id.' '.$checked.' name="category[]"><label style="color: #888888;" class="pref_label">' . $cat_name . '</label>';
                    }
                                $html .= $this->user_preference_tree_for_pref($id, $value->id);
