...
class EmployeeController < ApplicationController
  ...
  def att_report_all_excell
    ...
    @all_employee.each do |employee|
      ...
      new_book.worksheet(0).insert_row(i, temp)
      sheet1.row(i).default_format = center_align_format
    end  
    spreadsheet = StringIO.new 
    new_book.write spreadsheet 
    send_data spreadsheet.string, :filename => "All_report_employee.xls", :type =>  "application/vnd.ms-excel"
  end  
  ...
  def class_today
    ...
    unless @current_timetable.blank?
      ...
      if @date_to_use == @end_date
        ...
        @assignment_register = Assignment.find(:all,:conditions=>["((date(DATE_ADD(created_at,INTERVAL 6 HOUR)) = ? and  content like '%</%' ) or ( date(created_at) = ? and  content not like '%</%' )) and employee_id = ?",@date_to_use.to_date,@date_to_use.to_date,@employee.id])
        ...
        @entries += @current_timetable.timetable_entries.find(:all,:conditions=>{:subject_id=>elective_subjects},:include=>:class_timing,:order=>"class_timings.start_time")
        @attenadnce_register = SubjectAttendanceRegister.find(:all,:conditions=>["attendance_date >= ? and attendance_date <= ?",@date_to_use.to_date,@end_date.to_date])
        @assignment_register = Assignment.find(:all,:select=>"created_at,title,subject_id",:conditions=>["(( date(created_at) >= ? and date(created_at) <= ? and  content not like '%</%' )) and employee_id = ?",@date_to_use.to_date,@end_date.to_date,@employee.id])
        @assignment_register += Assignment.find(:all,:select=>"DATE_ADD(created_at,INTERVAL 6 HOUR) as created_at, title, subject_id",:conditions=>["((date(DATE_ADD(created_at,INTERVAL 6 HOUR)) >= ? and date(DATE_ADD(created_at,INTERVAL 6 HOUR)) <= ? and  content like '%</%' ) ) and employee_id = ?",@date_to_use.to_date,@end_date.to_date,@employee.id])
        # @assignment_register = Assignment.find(:all,:conditions=>["date(created_at) >= ? and date(created_at) <= ? and employee_id = ?",@date_to_use.to_date,@end_date.to_date,@employee.id])
        @classwork_register = Classwork.find(:all,:conditions=>["date(created_at) >= ? and date(created_at) <= ?",@date_to_use.to_date,@end_date.to_date])
        @events = Event.find(:all,:conditions=>["is_holiday = ? and is_common = ?",true,true])
      end  
      unless @entries.blank?
  ...
  def profile
    ...
    @gender = "Male"
    if !@employee.gender.blank? and @employee.gender.downcase == "f"
     @gender = "Female"
    end
    @status = "Active"
...
ORDER BY emp.first_name ASC"
  ...
  def employee_attendance
    ...
    if (@config.blank? or @config.config_value.blank? or @config.config_value.to_i != 1)
      ...
      @app_leaves = ApplyLeaveStudent.count(:conditions=>["(viewed_by_teacher is NULL or viewed_by_teacher=?) and forward=? and employee_id=?",false,true,@employee.id])
    end  
    @total_leave_count = @total_leave_count + @app_leaves
  end
