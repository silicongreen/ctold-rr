  ...
  def download_excell
    ...
    unless batch_id.nil?
      ...
      unless batchdata.blank?
        ...
        if student_class_name.blank?
          if classwork_publish_date.blank?
            @classworks =Classwork.find  :conditions=>"batches.name = '#{batch_name}'  and is_published=1 ",:order=>"classworks.created_at desc", :page=>params[:page], :per_page => 20,:include=>[{:subject=>[:batch]}]     
          else
            @pub_date = classwork_publish_date.to_datetime.strftime("%Y-%m-%d")
            @classworks =Classwork.find(:all,:conditions=>"batches.name = '#{batch_name}' and ( (DATE(DATE_ADD(classworks.created_at, INTERVAL 6 HOUR)) = '#{@pub_date}' and content like '%</%') OR ( ( (DATE(DATE_ADD(classworks.created_at, INTERVAL 6 HOUR)) = '#{@pub_date}' and content like '%</%') OR ( DATE(classworks.created_at) = '#{@pub_date}' and content not like '%</%' ) ) and content not like '%</%' ) )  and is_published=1 ",:order=>"classworks.created_at desc",:include=>[{:subject=>[:batch]}])  
          end
        elsif student_section.blank?
          if classwork_publish_date.blank?
            @classworks =Classwork.find(:all,:conditions=>"batches.name = '#{batch_name}' and courses.course_name = '#{student_class_name}'  and is_published=1 ",:order=>"classworks.created_at desc", :page=>params[:page], :per_page => 20,:include=>[{:subject=>[{:batch=>[:course]}]}])
          else
            @pub_date = classwork_publish_date.to_datetime.strftime("%Y-%m-%d")
            @classworks =Classwork.find(:all,:conditions=>"batches.name = '#{batch_name}' and ( (DATE(DATE_ADD(classworks.created_at, INTERVAL 6 HOUR)) = '#{@pub_date}' and content like '%</%') OR ( DATE(classworks.created_at) = '#{@pub_date}' and content not like '%</%' ) )  and courses.course_name = '#{student_class_name}'  and is_published=1 ",:order=>"classworks.created_at desc",:include=>[{:subject=>[{:batch=>[:course]}]}])
          end
        else
          batch = Batch.find_by_course_id_and_name(student_section, batch_name)
          unless batch.blank?
            if classwork_publish_date.blank?
              @classworks =Classwork.find(:all,:conditions=>"batches.id = '#{batch.id}'  and is_published=1 ",:order=>"classworks.created_at desc", :page=>params[:page], :per_page => 20,:include=>[{:subject=>[:batch]}])
            else
              @pub_date = classwork_publish_date.to_datetime.strftime("%Y-%m-%d")
              @classworks =Classwork.find(:all,:conditions=>"batches.id = '#{batch.id}'  and is_published=1 and ( (DATE(DATE_ADD(classworks.created_at, INTERVAL 6 HOUR)) = '#{@pub_date}' and content like '%</%') OR ( DATE(classworks.created_at) = '#{@pub_date}' and content not like '%</%' ) )",:order=>"classworks.created_at desc",:include=>[{:subject=>[:batch]}])
            end
          end
        end  
      end
      ...
      #@classworks =Classwork.paginate  :conditions=>"is_published=1",:order=>"classworks.created_at desc", :page=>params[:page], :per_page => 20,:include=>[{:subject=>[:batch]}] 
      if assignment_publish_date.blank?
        @classworks =Classwork.find(:all,:conditions=>"is_published=1",:order=>"classworks.created_at desc",:include=>[{:subject=>[:batch]}])
      else
        @pub_date = assignment_publish_date.to_datetime.strftime("%Y-%m-%d")
        @classworks =Classwork.find(:all,:conditions=>"is_published=1 and ( (DATE(DATE_ADD(classworks.created_at, INTERVAL 6 HOUR)) = '#{@pub_date}' and content like '%</%') OR ( DATE(classworks.created_at) = '#{@pub_date}' and content not like '%</%' ) )",:order=>"classworks.created_at desc",:include=>[{:subject=>[:batch]}])
      end 
    end   
