  ...
  def classes_view
    ...
    if @batches.length == 1
      ...
      @courses = Rails.cache.fetch("user_cat_links_courses_batches_#{batch_name}_#{current_user.id}_#{school_id}"){
        ...
        @tmp_courses = Course.find(:all, :conditions => ["courses.id IN (?) and batches.name = ? and courses.school_id = ?", @batch_ids, batch_name,school_id], :select => "courses.*,  GROUP_CONCAT(batches.id) as courses_batches", :joins=> "INNER JOIN `batches` ON batches.course_id = courses.id", :group => 'course_name', :order => "cast(replace(course_name, 'Class ', '') as SIGNED INTEGER) asc")
        @tmp_courses
      }
    end
  end
