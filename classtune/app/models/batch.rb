...
class Batch < ActiveRecord::Base
  ...
  named_scope :active,{ :conditions => ["batches.is_deleted = ? and batches.is_active = ? and courses.is_deleted = ?", false, true , false],:joins=>:course,:select=>"`batches`.*,CONCAT(courses.code,'-',batches.name) as course_full_name",:order=>"courses.course_name"}
  named_scope :inactive,{ :conditions => { :is_deleted => false, :is_active => false },:joins=>:course,:select=>"`batches`.*,CONCAT(courses.code,'-',batches.name) as course_full_name",:order=>"courses.course_name"}
  named_scope :deleted,{:conditions => { :is_deleted => true },:joins=>:course,:select=>"`batches`.*,CONCAT(courses.code,'-',batches.name) as course_full_name",:order=>"courses.course_name"}
  ...
  def full_name
    "#{name} #{course_name} #{section_name}"
  end
  
  def modified_full_name
    "#{course_name} #{section_name}, #{name} Shift"
  end
  ...
  def find_attendance_rank(start_date,end_date)
    ...
    unless @students.empty?
      ...
      unless working_days == 0
        ...
        student_percentages = []
        @students.each do|student|
          leaves = Attendance.find(:all,:conditions=>["student_id = ? and month_date >= ? and month_date <= ? and is_leave=?",student.id,start_date,end_date,false])
          absents = 0
          unless leaves.empty?
            leaves.each do|leave|
              working_days_count=self.find_working_days(leave.month_date.to_date,leave.month_date.to_date).select{|v| v<=leave.month_date.to_date}.count
  ...
  def importPreviousBatchSubject(course_id)
    @previous_batch = Batch.find(:first,:order=>'id desc', :conditions=>"batches.id < '#{self.id }' AND batches.is_deleted = 0 AND course_id = ' #{course_id }'",:joins=>"INNER JOIN subjects ON subjects.batch_id = batches.id  AND subjects.is_deleted = 0")
    unless @previous_batch.blank?
      ...
      subjects.each do |subject|
        if subject.elective_group_id.nil?
          Subject.create(:name=>subject.name,:code=>subject.code,:batch_id=>self.id,:no_exams=>subject.no_exams, :icon_number => subject.icon_number,
          ...
          elect_group_exists = ElectiveGroup.find_by_name_and_batch_id(ElectiveGroup.find(subject.elective_group_id).name,self.id)
  ...
  def importPreviousBatchFees(course_id)
    @previous_batch = Batch.find(:first,:order=>'id desc', :conditions=>"batches.id < '#{self.id }' AND batches.is_deleted = 0 AND course_id = ' #{course_id }'",:joins=>"INNER JOIN category_batches ON category_batches.batch_id = batches.id  INNER JOIN finance_fee_categories on finance_fee_categories.id=category_batches.finance_fee_category_id AND finance_fee_categories.is_deleted = 0 AND is_master= 1")
    unless @previous_batch.blank?
      ...
      categories.each do |c|
        ...
        unless particulars.blank? and fee_discounts.blank?
          new_category = CategoryBatch.new(:batch_id=>self.id,:finance_fee_category_id=>c.finance_fee_category_id)
          if new_category.save
            particulars.each do |p|
              receiver_id=p.receiver_type=='Batch' ? self.id : p.receiver_id
              new_particular = FinanceFeeParticular.new(:name=>p.name,:description=>p.description,:amount=>p.amount,\
                  :batch_id=>self.id,:receiver_id=>receiver_id,:receiver_type=>p.receiver_type)
              new_particular.finance_fee_category_id = new_category.finance_fee_category_id
              new_particular.save(false)
            end
            fee_discounts.each do |disc|
              ...
              discount_attributes['receiver_id']=self.id if disc.receiver_type=='Batch'
              discount_attributes["batch_id"]= self.id
              discount_attributes["finance_fee_category_id"]= new_category.finance_fee_category_id
              FeeDiscount.create(discount_attributes)
            end
          end
        end
      end
