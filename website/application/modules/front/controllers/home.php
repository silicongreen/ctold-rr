...
//                }
    ...
    private function check_user_valid($user_type) {
        if (free_user_logged_in()) {
    ...
    private function update_user_before_apply($user_data,$u_array)
    {
    ...
    private function get_teacher_default_data($user_data,$u_id)
    {
        $data['user_id'] = $u_id;
    ...
    private function get_parent_default_data($user_data,$u_id)
    {
        $data['user_id'] = $u_id;
        $data['country_id'] = 14;
        $data['school_id'] = $user_data->paid_school_id;
        ...
        if ($user_data = $this->check_user_valid($user_type)) {
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $this->update_user_before_apply($user_data,$this->input->post());
                    //insert user data
                    ...
                    if($u_id)
                    {
                        $ward = get_parent_children($this->input->post("admission_no"),$user_data);
                        $this->db->dbprefix = '';
                        $st = $this->get_parent_default_data($user_data, $u_id);
    ...
    public function createGuardianStudent($s_array,$n_g,$sid,$user_data)
    {
        ...
        if ($user_data = $this->check_user_valid($user_type)) {
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    {
                        $this->db->dbprefix = '';
                        $st = $this->get_user_default_data($user_data, $u_id);
                        ...
                        $st['phone1'] = $this->input->post("phone1");
                        $st['gender'] = $this->input->post("gender");
                        $st['address_line1'] = $this->input->post("address_line1");
            ...
            $s_content = $this->load->view('apply_for_student_admission', $data, true);
    ...
    public function apply_for_teaher_admission() {
        $user_type = array(3);
        if ($user_data = $this->check_user_valid($user_type)) {
            $ar_js = array();
            $ar_css = array();
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    {
                        $this->db->dbprefix = '';
                        $st = $this->get_teacher_default_data($user_data, $u_id);
            ...
            $s_content = $this->load->view('apply_for_teacher_admission', $data, true);
