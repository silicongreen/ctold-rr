...
class Observations extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_active, observation_group_id, sort_order, school_id', 'numerical', 'integerOnly'=>true),
			array('name, desc', 'length', 'max'=>255),
			array('created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, desc, is_active, observation_group_id, created_at, updated_at, sort_order, school_id', 'safe', 'on'=>'search'),
		);
	}
