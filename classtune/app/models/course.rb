...
class Course < ActiveRecord::Base
  ...
  #validates_uniqueness_of :code,:scope=> [:is_deleted],:if=> 'is_deleted == false', :unless => Proc.new { |course| course.new_batch == true}
  ...
  named_scope :active_courses, :select => "courses.*, GROUP_CONCAT(courses.section_name) as sections", :conditions => { :is_deleted => false }, :group => 'course_name', :order => "cast(replace(course_name, 'Class ', '') as SIGNED INTEGER) asc"
  ...
  def find_batches_data(batch_name = nil, course_name = nil, id = nil, batch_id = nil)
    unless batch_id.nil?
