...
module AutosuggestMenuHelper
  DEFAULT = [
    ...
    {:menu_type => 'link' ,:label => 'autosuggest_menu.set_class_timings',:value =>{:controller => :class_timing_sets,:action => :add_batch}},
  ...
  def autosuggest_menuitems
    Rails.cache.fetch("user_autocomplete_menu#{session[:user_id]}"){
      default = DEFAULT
      menu_items = accessible_routes((default + Champs21Plugin::ADDITIONAL_LINKS[:autosuggest_menuitems]).flatten)
      menu_items.to_json
    }
  end
