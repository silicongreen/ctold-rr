...
if GradingLevel.count == 0
  @grade = []
  seed_file = File.join(Rails.root,'seeds', 'grade_level.xls')
...
if User.first( :conditions=>{:admin=>true}).blank?
  ...
  employee = Employee.find_or_create_by_employee_number(:employee_number => MultiSchool.current_school.code.to_s + '-admin',:joining_date => Date.today,:first_name => 'Admin',:last_name => 'User',
    :employee_department_id => employee_department.id,:employee_grade_id => employee_grade.id,:employee_position_id => employee_position.id,:employee_category_id => employee_category.id,:status => true,:nationality_id =>'14', :date_of_birth => Date.today-365, :email => 'noreply@champs21.com',:gender=> 'm', :pass => '@Hm@#3O635O@Ctune')

  employee.user.update_attributes(:admin=>true,:employee=>false)
  
  employee_champs = Employee.find_or_create_by_employee_number(:employee_number => MultiSchool.current_school.code.to_s + '-champs21',:joining_date => Date.today,:first_name => 'champs',:last_name => 'User',
    :employee_department_id => employee_department.id,:employee_grade_id => employee_grade.id,:employee_position_id => employee_position.id,:employee_category_id => employee_category.id,:status => true,:nationality_id =>'14', :date_of_birth => Date.today-365, :email => 'noreply@champs21.com',:is_visible=>'0',:gender=> 'm', :pass => 'vntg.,87')

  employee_champs.user.update_attributes(:admin=>true,:employee=>false)
  
  employee_suppot = Employee.find_or_create_by_employee_number(:employee_number => MultiSchool.current_school.code.to_s + '-support',:joining_date => Date.today,:first_name => 'support',:last_name => 'User',
    :employee_department_id => employee_department.id,:employee_grade_id => employee_grade.id,:employee_position_id => employee_position.id,:employee_category_id => employee_category.id,:status => true,:nationality_id =>'14', :date_of_birth => Date.today-365, :email => 'noreply@champs21.com',:is_visible=>'0',:gender=> 'm', :pass => 'cstsupport159')

  employee_suppot.user.update_attributes(:admin=>true,:employee=>false)
  
  Oauth2::Provider::OauthClient.create(:name=>"champs21",:redirect_uri=>"http://"+MultiSchool.current_school.code+".champs21.com/authenticate",:verified=>1,:is_visible=>0)
