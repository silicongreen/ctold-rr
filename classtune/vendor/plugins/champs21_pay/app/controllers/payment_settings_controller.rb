              ...
              if params[:export].to_i == 1
                ...
                @online_payments.each do |payment|
                  ...
                  if @gateway == "trustbank"
                    ...
                    row_new = [payment.gateway_response[:Message][:OrderID], payment.order_id, payment.payee.admission_no, payment.payee.full_name, payment.gateway_response[:Message][:SessionID], Champs21Precision.set_and_modify_precision(amt), Champs21Precision.set_and_modify_precision(fee_percent), Champs21Precision.set_and_modify_precision(total_amount), payment.gateway_response[:Message][:OrderStatus], I18n.l((payment.transaction_datetime.to_time).to_datetime,:format=>"%d %b %Y %H:%M:%S")]
                  elsif @gateway == "bkash"
