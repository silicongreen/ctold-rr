class MarksController < ApplicationController
  ...
  def get_exams
    ...
    if data_type.to_i == 8
      ...
      @exam_connect = ExamConnect.active.find(:all,:conditions=>"batch_id="+batch_id+" and (result_type=1 or result_type=2 or result_type=5  or result_type=6  or result_type=7  or result_type=8  or result_type=10  or result_type=11  or result_type=13  or result_type=14  or result_type=15 or result_type=16 or result_type=17) and is_deleted=0")
    else
          ...
          if !@subjects.include?(exam.subject_id) 
            if @employee_subjects.include?(exam.subject_id) or @current_user.admin?
              ...
              if MultiSchool.current_school.id == 340 and sjws_exam_type_array.include?(exam_connect.result_type.to_i) and (current_user.admin? or current_user.employee_record.is_advisor)
                ...
                data[k][1] = @template.link_to(exam_connect.name+"(Comment Entry)", '/exam/' + 'comment_tabulation/' +exam_connect.id.to_s+'?blank_page=1', :target => "_blank")
              elsif MultiSchool.current_school.id != 361 and exam_connect.result_type.to_i > 13 and (@current_user.admin? or (!@batches2.blank? and @batches2.include?(exam_connect.batch_id)))
                data[k][1] = @template.link_to(exam_connect.name+"(Comment Entry)", '/exam/' + 'comment_tabulation/' +exam_connect.id.to_s+'?blank_page=1', :target => "_blank")
              else
        ...
        if !@subjects.include?(exam.subject_id) 
          if @employee_subjects.include?(exam.subject_id) or @current_user.admin?
            ...
            if school_id == 340
              ...
              unless c_exam_array.include?(exam_connect.id.to_i)
                data[k] = []
                if ((exam_connect.result_type == 17 or exam_connect.result_type == 19 or exam_connect.result_type == 20) and  school_id != 352)
                  data[k][0] = exam_connect_batch.to_s
