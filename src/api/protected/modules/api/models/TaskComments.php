...
class TaskComments extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, task_id, attachment_file_size, school_id', 'numerical', 'integerOnly'=>true),
			array('attachment_file_name, attachment_content_type', 'length', 'max'=>255),
			array('description, attachment_updated_at, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, task_id, description, attachment_file_name, attachment_content_type, attachment_file_size, attachment_updated_at, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
