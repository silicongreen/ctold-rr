...
class EmployeeLeaveTypes extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('status, carry_forward, school_id', 'numerical', 'integerOnly'=>true),
			array('name, code, max_leave_count', 'length', 'max'=>255),
			array('created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, code, status, max_leave_count, carry_forward, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
