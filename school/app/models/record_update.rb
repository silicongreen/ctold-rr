  ...
  class << self
    def update_normal_run
      ...
      RecordUpdate.transaction do
        ...
        Champs21Plugin::AVAILABLE_MODULES.each do |m|
          plugin_records_update_files = Dir["#{Rails.root}/vendor/plugins/#{m[:name]}/db/seeds/*.rb"].sort
    ...
    def update_school_run(s_id)
      ...
      RecordUpdate.transaction do
        ...
        #plugins
        Champs21Plugin::AVAILABLE_MODULES.each do |m|
          plugin_records_update_files = Dir["#{Rails.root}/vendor/plugins/#{m[:name]}/db/seeds/school/*.rb"].sort
          plugin_records_update_files.each do |r_u|
            if confirmed_task(File.basename(r_u), s_id)
              begin
                load(r_u)
