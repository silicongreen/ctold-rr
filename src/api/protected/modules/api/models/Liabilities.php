...
class Liabilities extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_solved, is_deleted, school_id', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>255),
			array('amount', 'length', 'max'=>10),
			array('description, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, description, amount, is_solved, is_deleted, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
