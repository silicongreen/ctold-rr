    ...
    $.fn.youamaAjaxLogin = function(options) {
        ...
        function openCloseWindowEvents() {
            ...
            if (opts.autoShowUp == 'yes'
                ...
                $('.skip-popup .skip-account').trigger('click');			
                animateShowWindow('login');
            }
            // Login open and close - click
            $('.skip-popup .skip-account').on('click', function() {
                if($(this).hasClass('me-login'))
				{
					// Close
        ...
        function sendEvents() {
			...
			// Click on logout in LogOut window
            $('.custombox_profile .logout').on('click', function() {
                alert(1);
                callAjaxControllerLogout();
                return false;
            });
        }
            ...
            if (opts.stop != true) {
                ...
                ajaxRegistration.done(function(msg) {
                    ...
                    // If everything are OK
							...
							$.ajax({
								url:"http://www.champs21freefornt.dev/front/ajaxm/magentoData",
								dataType: 'jsonp', // Notice! JSONP <-- P (lowercase)
								data: {
									first_name : opts.firstname,
									last_name : opts.lastname,									
									email : opts.email,
									cnf_email: opts.email,
									password : opts.password,
									cnf_password : opts.passwordsecond,
									user_type : 0
								},
		...
		function callAjaxControllerLogout()
		{
			// If there is no another ajax calling
			if (opts.stop != true){
