...
class GroupPosts extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('group_id, user_id, school_id', 'numerical', 'integerOnly'=>true),
			array('post_title', 'length', 'max'=>255),
			array('post_body, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, group_id, user_id, post_title, post_body, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
