...
class StudentController < ApplicationController
  ...
  def student_history_excell
      ...
      if @history_type.to_i == 1
        ...
        @students = ArchivedStudent.find(:all,:conditions=>["date_of_leaving between ? AND ?",@from_date,@to_date],:include=>[{:batch=>[:course]},:student_category])
      end  
...
#    end
  ...
  def student_photo_download_all
    require 'zip/zipfilesystem'
    zip_name = "school_photo.zip"
...
#                      end
  ...
  def download_all_profile_pdf
    ...
    @students = Student.find(:all,:conditions=> condition,:include=>[{:batch=>[:course]}],:order => order_str)
    render :pdf => "download_all_profile_pdf",
      :orientation => 'Portrait',
      :margin => {:top=> 0,
      :bottom => 0,
      :left=> 10,
  ...
  def fees
    if MultiSchool.current_school.classpay_enabled
      ...
      if !@data.blank? and !@data['class_pay'].blank? and !@data['class_pay_key'].blank? and !@data['class_pay_id'].blank? and MultiSchool.current_school.classpay_active
        redirect_to "https://classpay.com.bd/main/login/login_via_key?key="+@data['class_pay_key'].to_s+"&user_id="+@data['class_pay_id'].to_s
      end
