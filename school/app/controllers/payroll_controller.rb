      ...
      if payroll_created.empty?
        if request.post?
          ...
          params[:manage_payroll].each_pair do |k, v|
            ...
            EmployeeSalaryStructure.create(:employee_id => params[:id], :payroll_category_id => k, :amount => Champs21Precision.set_and_modify_precision(current_amount))
          end
    ...
    render :update do |page|
      @dependent_categories.each do |c|
        unless c.percentage.nil?
          ...
          calculated_amount = Champs21Precision.set_and_modify_precision(amount.to_f*percentage_value/100)
      ...
      params[:manage_payroll].each_pair do |k, v|
        ...
        unless row_id.nil?
          EmployeeSalaryStructure.update(row_id, :employee_id => params[:id], :payroll_category_id => k,
            :amount => Champs21Precision.set_and_modify_precision(current_amount).to_f)
        else
