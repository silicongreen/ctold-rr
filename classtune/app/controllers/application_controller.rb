...
class ApplicationController < ActionController::Base
  ...
  def get_tabulation_connect_exam(connect_exam_id,batch_id,all_class_report=false,failed_list = false,is_unsolved = 0)
    ...
    if all_class_report && failed_list != false
      request.set_form_data({"connect_exam_id"=>connect_exam_id,"batch_id"=>batch_id,"all_class_report"=>all_class_report,"failed_list"=>failed_list,"is_unsolved"=>is_unsolved,"call_from_web"=>1,"user_secret" =>session[:api_info][0]['user_secret']})
    elsif all_class_report
      request.set_form_data({"connect_exam_id"=>connect_exam_id,"batch_id"=>batch_id,"all_class_report"=>all_class_report,"call_from_web"=>1,"is_unsolved"=>is_unsolved,"user_secret" =>session[:api_info][0]['user_secret']})
    else
      request.set_form_data({"connect_exam_id"=>connect_exam_id,"batch_id"=>batch_id,"call_from_web"=>1,"is_unsolved"=>is_unsolved,"user_secret" =>session[:api_info][0]['user_secret']})
    end
...
#      end
  ...
  def current_user
    unless session[:user_id].nil?
      user = User.active.find(:first, :conditions => "id = #{session[:user_id]}") 
      user
    end
  end
