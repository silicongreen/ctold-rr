    ...
    t.Notify = function() {
        function t(t) {
            for (var s = e.length - 1; s >= 0; s--) {
                var i = e[s];
                i.close(), e.splice(s, 1)
            }
        }
        if (!s.isSupported) return void(this.requestPermission = this.create = this.closeAll = function() {});
        var e = [];
        this.requestPermission = function(t) {
            switch (s.permissionLevel()) {
                case s.PERMISSION_DENIED:
                    break;
                case s.PERMISSION_GRANTED:
                    break;
                case s.PERMISSION_DEFAULT:
                default:
                    s.requestPermission()
            }
        }, this.create = function(i, a) {
            if (s.permissionLevel() === s.PERMISSION_GRANTED) {
                a.icon = a.icon || "empty", a.tag = a.tag || "global", t(a.tag), i = "" + i, a.body = "" + a.body;
                var n = s.createNotification(i, a);
                e.push(n);
                var o = n.notification;
                return o.onclose = o.onclick = function() {
                    var t = e.indexOf(n); - 1 !== t && e.splice(t, 1)
                }, n
            }
        }, this.closeAll = function() {
            for (var t = e.length - 1; t >= 0; t--) e[t].close();
            e = []
        };
        var i = this;
    ...
    t.CannedMessagesModel = Backbone.Model.extend({
        ...
        deleteMessage: function(t) {
            ...
            e.post(s.deleteCannedMessagePath, {
                ...
                t.success ? n.trigger("change:messages message:deleted") : n.trigger("message:deleteError")
            })
        },
...
function(t, e, s, i) {
    var a = t.AdminChatModel = Backbone.Model.extend({
        ...
        checkUsers: function(t) {
            var i = this,
                a = e.get(s.getOnlineUsersPath, function(t) {
                    t.success && i.trigger("users:online", t.users)
                });
            t && a.always(t)
        },
        keepAlive: function(t) {
            var i = e.get(s.keepAlivePath);
            t && i.always(t)
        },
        ...
        getTypingStatus: function(t) {
            if (this.talkingUsersIds.length > 0) {
                var i = this,
                    a = e.post(s.getTypingStatusPath, {
                        ids: this.talkingUsersIds
                    }, function(t) {
                        if (t.success) {
                            var e = i.filterTyping(t.results);
                            e.length > 0 && i.trigger("users:typing", e)
                        }
                    });
                t && a.always(t)
            } else t && t()
        ...
        getMessages: function(t) {
            var i = this,
                a = e.get(s.newMessagesPath, function(t) {
                    t = t.messages, t.length > 0 && (t = i.filterNewMessages(t), i.loadUsersData(t, function() {
                        i.trigger("messages:new", t)
                    }))
                });
            t && a.always(t)
        },
        ...
        deleteOperator: function(t) {
            ...
            e.post(s.deleteOperatorPath, t, function(t) {
                t.success ? i.trigger("change:operators operator:deleted") : i.trigger("operator:deleteError")
            })
        },
        ...
        manageConnection: function() {
            function t() {
                e.prevRequestsComplete() && (e.resetRequestsStatus(), e.user.hasRole("OPERATOR") ? (e.getMessages(function() {
                    e.requestsStatus.getMessages = !0
                }), e.keepAlive(function() {
                    e.requestsStatus.keepAlive = !0
                }), e.getTypingStatus(function() {
                    e.requestsStatus.getTypingStatus = !0
                })) : e.requestsStatus.getMessages = e.requestsStatus.keepAlive = e.requestsStatus.getTypingStatus = !0, e.checkUsers(function() {
                    e.requestsStatus.checkUsers = !0
                }))
            }
        ...
        prevRequestsComplete: function() {
            if (Date.now() - this.requestInitTime > a.REQUEST_STATUS_RESET_TIMEOUT) return this.initRequestsStatus(), !0;
    ...
    var i = /(\w+:\/\/)?([\da-z\.\-@]+)\.([a-z\.]{2,})([?&=%#;\/\w\.-]*)*\/?/g,
        a = t.MessageView = Backbone.View.extend({
            ...
            updateTime: function(t) {
                ...
                var d = o > 0 ? c : n > 0 ? n + " " + e.ui.timeDaysAgo : a > 0 ? a + " " + e.ui.timeHoursAgo : i > 0 ? i + " " + e.ui.timeMinutesAgo : Math.max(s - s % 5, 1) + " " + e.ui.timeSecondsAgo;
                if (this.$time.html(d), t) {
...
function(t, e, s) {
    t.MenuView = Backbone.View.extend({
        ...
        showLogs: function(e) {
            var s = 18 + this.$sideLogs[0].offsetTop;
            this.$sideMarker.animate({
                top: s
...
function(t, e, s) {
    t.ChatTabView = Backbone.View.extend({
        ...
        updateUnreadCounter: function(t) {
            ...
            this.unreadTalks = Math.max(0, this.unreadTalks + t), 1 === e && 0 === this.unreadTalks ? this.trigger("talks.read") : this.trigger("talks.unread")
        }
    })
}(window.Application, jQuery, window.chatConfig),
function(t, e, s, i) {
    var a = t.ChatView = Backbone.View.extend({
        ...
        initialize: function() {
            this.settings = t.model.settings, this.chat = t.model.chat, this.user = t.model.user, this.parentView = this.options.parentView, this.chatBox = new t.ChatBoxView({
        ...
        handleNewMessages: function(e, s) {
            if (i.each(e, this.handleMessage, this), !s) {
                e.length > 0 && this.settings.get("sound") && t.service.soundPlayer.play("message");
                var a = e[e.length - 1];
                a.info = this.model.get("info");
                var n = new t.MessageModel(a),
                    o = this;
                setTimeout(function() {
                    t.service.notify.create("PHP Live Support Chat (" + o.parentView.unreadTalks + ")", {
                        body: n.getReadableName() + ": " + n.get("body")
                    })
                }, 0)
            }
        },
    ...
    t.SettingsView = Backbone.View.extend({
        ...
        validate: function(s, i) {
            ...
            this.$inputs.each(function() {
                ...
                switch (t.removeClass("customer-chat-input-error"), t.data("validator")) {
                    case "color":
                        0 != t.val().length && a.colorExp.test(t.val()) || (t.addClass("customer-chat-input-error"),
                            a.errors = !0);
                        break;
                    case "mail":
        ...
        save: function() {
            if (this.isValid()) {
                ...
                this.$inputs.each(function() {
                    var s = e(this);
                    "checkbox" == s.attr("type") ? t[s.attr("name")] = s.is(":checked") ? "true" : "false" : t[s.attr("name")] = s.val()
...
function(t, e, s) {
    t.WidgetThemeView = Backbone.View.extend({
        ...
        save: function() {
            ...
            this.$inputs.each(function() {
                var e = s(this);
                "checkbox" == e.attr("type") ? t[e.attr("name")] = e.is(":checked") ? "true" : "false" : t[e.attr("name")] = e.val()
...
function(t, e, s, i) {
    t.OperatorsView = Backbone.View.extend({
        ...
        render: function() {
            this.$operators.html("");
            var i = this;
            var  op_types = ['Markating Operator','Sales Operator','Technical Operator'];
        
            s.each(this.model.getOperators(), function(s) {
                var op_t = s.op_type;
                var a = e(t.template.operatorItem);
                s.image && i.setEditAvatarIcon(a, s.image), a.find(".customer-chat-operator-name").html(s.name), 
                a.find(".customer-chat-operator-mail").html(s.mail), 
                a.find(".customer-chat-operator-type").html(op_types[op_t-1]), 
                a.find(".customer-chat-operators-edit").data("id", s.id), a.find(".customer-chat-operators-remove").data("id", s.id), i.$operators.append(a)
            })
        },
        ...
        showEdit: function(s, a) {
            ...
            "pass" !== o ? (r = l ? l.data("id") : a, h = t.model.selectedOperator = t.model.chat.getOperator(r)) : (h = t.model.selectedOperator, r = h.id), this.$edit.find(".customer-chat-tabs-header").html(this.user.get("id") === a ? "Edit profile" : "Edit operator"), h && (this.$edit.find("#usernameField").val(h.name), 
            this.$edit.find("#mailField").val(h.mail),this.$edit.find("#op_type").val(h.op_type), h.image && n.setEditAvatarIcon(this.$edit, h.image));
            var c = t.view.dialogs;
            this.$avatarInput.ajaxfileupload({
                ...
                onStart: function() {
                    c.message("Uploading", "Uploading image, please wait...")
                },
                onComplete: function(t) {
                    c.message("Avatar uploaded", "Uploaded successfully"), h.image = t.image, n.model.saveOperator(h), n.setEditAvatarIcon(n.$edit, h.image)
                }
        ...
        showAdd: function() {
            this.state = "add", this.$edit.find(".customer-chat-tabs-header").html("Add new operator"), this.$edit.find(".edit-only, .pass-only").hide(), this.$edit.find(".add-only").show(), this.$edit.find(".customer-chat-input-error").removeClass("customer-chat-input-error"), t.model.selectedOperator = {
                roles: ["OPERATOR"]
            }, this.$edit.find("#usernameField").val(""), this.$edit.find("#mailField").val(""), this.$edit.find("#op_type").val("")
            , this.$edit.find("#passField").val(""), this.$edit.find("#rePassField").val(""), this.$list.hide(), this.$edit.show()
        },
        ...
        save: function() {
            if (this.isEditValid() && !this.syncing) {
                ...
                if (e) {
                    "edit" == this.state || "add" == this.state ? (e.name = this.$edit.find("#usernameField").val(), e.mail = this.$edit.find("#mailField").val(), e.op_type = this.$edit.find("#op_type").val(), e.password = this.$edit.find("#passField").val()) : "pass" == this.state && (e.currPassword = this.$edit.find("#changePassCurrentField").val(), e.password = this.$edit.find("#changePassField").val());
                    var i = this;
                    t.model.chat.once("operator:saved", function(t) {
        ...
        save: function() {
            if (this.isEditValid() && !this.syncing) {
                ...
                if (e) {
                    ...
                    var i = this;
                    this.model.once("message:saved", function(t) {
                        i.enable(), i.showList()
    ...
    var i = t.HistoryView = Backbone.View.extend({
        ...
        search: function() {
            if (!this.searching) {
                ...
                this.$inputs.each(function() {
                    var s = e(this);
                    s.val() && ("checkbox" == s.attr("type") ? t[s.attr("name")] = s.is(":checked") ? "true" : "false" : t[s.attr("name")] = s.val())
        ...
        addSearchResult: function(i) {
            var a = i[0],
                ...
                c = new t.MessageModel(a);
            n.find(".customer-chat-history-item-username").html(c.getTalkName()), n.find(".customer-chat-history-item-time").html(l);
            var d = s.clone(c.attributes);
            d.author = d.author + " (" + d.authorMail + ")", n.data("info", i), this.$resultItems.append(n), this.$results.mCustomScrollbar("update")
        },
...
function(t, e, s) {
    var i = t.WindowView = Backbone.View.extend({
        ...
        stopMsgIndicator: function() {
            this.msgIndicatorTimer && (clearInterval(this.msgIndicatorTimer), document.title = this.titleText, delete this.msgIndicatorTimer), t.service.notify.closeAll()
        },
