...
class Min extends CI_Controller {
			...
			switch($segments)
			{
				case $this->config->item('js_route_segment'):
					...
					break;
                                ...
                                case $this->config->item('css_route_segment') . "1":
					$this->_type = 'css';
					break;
	...
	private function _display($files)
	{
		...
		$cache_name = str_replace('/','_', $this->_request); // this is why we needed that request var
                ...
                $cache_name = "Minify-" . strtoupper(str_replace("." . $this->_type, "", $cache_name)) . "-" . strtoupper($this->_type);
//                if ( $this->_type == "css" )
//                {
...
//                }
                if (!$cache = $this->read_from_cache($cache_name))
		{
                ...
                {
                    $headers['Content-Length'] = strlen($cache);
	...
	private function _minify($files=Array(), $cache_name)
	{
		...
		else
		{
			...
			return Minify::serve( 'Files', $options, $this->_type, $cache_name ) ;
		}
	}
	...
	function write_to_cache($name, $value)
	{
            ...
            if ($this->config->item('use_ci_cache'))
            {
                $this->cache->save($value, $name, $this->config->item($this->_type.'_cache_max_age'));
            }
	}
