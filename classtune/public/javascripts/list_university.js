...
function make_li(li_class,li_type,li_text,li_val){
    ...
    check_box.name = "university[university_"+li_type+"][]";
...
function show_submenu(menu_items,type,parent,check){
    if($(type+'_'+li.down().value)!=null){
        ...
        if(type == 'edu_district'){
            ...
            $$('.'+type+'_button').invoke('observe','click',load_university);
        }
        else if(type == 'university'){
            $('university_list').insert(ul);
            $$('#university_list input').invoke('observe','change',check_checker);
        }
...
function load_education_district(){
    ...
    $$('#university_list ul').invoke('hide');
    l_array = new Array();
...
//make a university menu. Also see the javascripts/statebodies.js.erb in the view folder
function load_university(){
    li = this.up();
    check =  this.previous(1).checked;
    id = li.down().value;
    type = "university";
    dest = $('university_list');
    $$('#university_list ul').invoke('hide');
    if($('university_'+id) == null){        
        l_array = new Array();
