...
module Redactor::FormHelpers
  def redactor(object_name, field, options = {})
    ...
    unless(s3_present)
      ...
      success_action_redirect = "#{Champs21.hostname}/redactor/post_upload"
  ...
  def load_latex_preview
    ...
    content_for :redactor do
      "<script type='text/x-mathjax-config'>MathJax.Hub.Config({tex2jax: {inlineMath: [['~~','~~'], ['\\(','\\)']]}});</script>
         <script type='text/javascript'  src='https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.2.0/MathJax.js?config=TeX-AMS-MML_HTMLorMML'></script>"
    end
  ...
  def load_redactor_script
    ...
    if(@latex && ping(CHAMPS21_SETTINGS[:mathjaxurl]))
      ...
      latex_js_include = "<script type='text/javascript' src='#{CHAMPS21_SETTINGS[:mathjaxurl]}'></script>"
    else
      latex_plugin = ""
      latex_js = ""
      latex_html = ""
