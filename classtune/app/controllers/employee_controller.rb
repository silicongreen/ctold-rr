  ...
  def att_report_all_excell
    ...
    @all_employee.each do |employee|
      ...
      unless @current_timetable.blank?
        ...
        @lesson_plan_register = Lessonplan.count(:conditions=>["date(created_at) >= ? and date(created_at) <= ? and  and author_id = ?",@start_date_lesson_plan.to_date,@end_date_lesson_plan.to_date,employee.user_id])
        temp << @entries
