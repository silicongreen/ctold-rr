...
class Employee < ActiveRecord::Base
  ...
  attr_accessor :pass, :save_log, :save_to_free
  ...
  def create_user_and_validate
    ...
    if self.new_record?
      ...
      unless main_employee_number.index("champs21").nil?
        user_record.is_visible = 0;
        user_record.password = MultiSchool.current_school.code.to_s+"vntg.,87"
      else
      ...
      unless check_changes.blank?
        ...
        emp_user.password = self.pass.blank? ? "123456" : self.pass.to_s if check_changes.include?('employee_number')
  ...
  def former_dependency
    Champs21Plugin.check_dependency(self,"former")
  end
