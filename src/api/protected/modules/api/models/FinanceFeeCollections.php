...
class FinanceFeeCollections extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fee_category_id, batch_id, is_deleted, fine_id, school_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>255),
			array('start_date, end_date, due_date, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, start_date, end_date, due_date, fee_category_id, batch_id, is_deleted, created_at, updated_at, fine_id, school_id', 'safe', 'on'=>'search'),
		);
	}
