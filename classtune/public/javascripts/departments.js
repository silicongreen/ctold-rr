...
var TablePrograms = function () {
    ...
    var handleTable = function () {
        ...
        jq(document).on("click","#new_department",function(e){
            ...
            jq("#name").focus();                        
            jq("#faculty-name-select").select2("val", "");
            ...
            jq('.modal-title').text('New Department'); // Set Title to Bootstrap modal title
        ...
        table.on('click', '.edit', function (e) {
            ...
            var url = "/setup/department_by_id"; 
            var formData = {
                    'department_id'  : id
            };
            ...
            jq.ajax({
                ...
                success: function(data)
                {
                    ...
                    jq("#faculty-name-select").select2("val", "");
                    jq('#faculty-name-select').val(data.department.faculty_id); // Change the value or make some change to the internal state
                    jq('#faculty-name-select').trigger('change.select2'); // Notify only Select2 of changes
                    
                    jq("#campus-name-select").select2("val", "");
                    jq('#campus-name-select').val(data.department.campus_id); // Change the value or make some change to the internal state
                    jq('#campus-name-select').trigger('change.select2'); // Notify only Select2 of changes
                    
                    jq('#name').val(data.department.name);                   
                    
                    jq('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                    jq('.modal-title').text('Edit Department'); // Set title to Bootstrap modal title
        ...
        jq(document).on("click","#btnSave",function(e){
            ...
            if(save_method == 'add') {
                url = "/setup/save_department";
                department_data += jq('[name="faculty-name-select"]').val() + "++" + jq('[name="campus-name-select"]').val() + "++" + jq('[name="name"]').val() + ",,";
            } else {
                url = "/setup/update_department";
                department_data += jq('[name="faculty-name-select"]').val() + "++" + jq('[name="campus-name-select"]').val() + "++" + jq('[name="name"]').val() + "++" + jq('#edit_id').val() + ",,";
            }
                ...
                {   
                    ...
                    if ( a_data[0] == "SAVE" )          
                    {
                        ...
                        if(save_method == 'add') {      
                            ...
                            var fff = jq('[name="faculty-name-select"]').select2('data');
                            ...
                            oTable.cell(nRow, 3).data(jq('[name="name"]').val());
        ...
        jq(document).on("click",".delete",function(e){
            ...
            if(confirm('Are you sure delete this data?'))
            {
                ...
                var url = "/setup/delete_department"; 
                var formData = {
                        'id'  : id
                };
                jq.ajax({
        ...
        function checkValidation()
        {
            ...
            var check = true;
            if ( jq.trim(jq('[name="faculty-name-select"]').val()).length == 0 )
            {
                check = false;
                if(jq(".select2-container").parent().find(".error").length == 0)
                {
                    jq(".select2-container").css("border","1px solid red");
                    jq(".select2-container").after( "<p class='error'>Select a faculty</p>" );
                }                
            }
            else
            {
            ...
            if ( jq.trim(jq('[name="campus-name-select"]').val()).length == 0 )
            {
                check = false;
                if(jq(".select2-container").parent().find(".error").length == 0)
                {
                    jq(".select2-container").css("border","1px solid red");
                    jq(".select2-container").after( "<p class='error'>Select a campus</p>" );
                }                
            }
            else
            {
