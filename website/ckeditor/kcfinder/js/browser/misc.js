...
browser.showDialog = function(e) {
    ...
    //$('#dialog').draggable({handle: '#dialog div.title'});
    ...
    if (e) {
        ...
        {
            $('#dialog').css({
                left: parseInt((( $("#main_container").width() - 120) - $('#dialog').outerWidth()) / 2) + 'px',
                top: parseInt((($(window).height() - 20) - $('#dialog').outerHeight()) / 2) + 'px'
            });
        }
        else
        {
...
browser.hideDialog = function() {
    ...
    if ( title == "Gallery Management" )
    {
        $.ajax({
            type: 'GET',
            dataType: 'json',
            data: {"range" : $("#range_data").html(), search_text: $("#srch").val()},
...
browser.showAlert = function(shadow) {
    ...
    if ( form_gallery == 1 )
    {
        var left = parseInt((( $("#main_container").width() - 120) - $('#dialog').outerWidth()) / 2);
        var top  = parseInt((($(window).height() - 20) - $('#dialog').outerHeight()) / 2);
    }
    else
    {
...
browser.confirm = function(question, callBack) {
    ...
    $('#dialog').html('<div class="box" style="height: 100px;"><div class="question">' + browser.label(question) + '<div class="buttons"><button>' + browser.label("No") + '</button>&nbsp;&nbsp;&nbsp;&nbsp; <button style="margin-left: 10px;">' + browser.label("Yes") + '</button></div></div></div>');
    browser.showDialog();
...
browser.shadow = function() {
    var pos = $("#from_gallery").val();
    if ( pos == 1 )
    {
        $('#main_shadow').css('display', 'block');
    }
    else
    {
...
browser.showMenu = function(e) {
    ...
    if ( form_gallery == 1 )
    {
        left = left - 217;
        top = top - 21;
        if (($('#dialog').outerWidth() + left) > ( $("#main_container").width() - 120))
            left = ( $("#main_container").width() - 80) - $('#dialog').outerWidth();
        if (($('#dialog').outerHeight() + top) > $(window).height())
            top = $(window).height() - $('#dialog').outerHeight();
    }
    else
...
browser.fileCaptionDialog = function(e, post, inputName1, inputName2, url, labels, callBack, selectAll) {
    var cap = "";
    var source = "";
    $.ajax({
        type: 'POST',
        dataType: 'json',
        data: post,
        url: browser.baseGetData('getcaption'),
        async: false,
        success: function(data) {
            cap = data.caption;
            source = data.source;
    ...
    var html = '<form method="post" action="javascript:;">' +
        '<div class="box">' +
        '<input name="' + inputName1 + '" type="text" value="' + cap + '" /><br />' +
        '<input name="' + inputName2 + '" type="text" value="' + source + '" /><br />' +
    ...
    $('#dialog form').submit(function() {
        ...
        eval('post.caption = name.value; post.source = source.value;');
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: url,
...
browser.baseGetData = function(act) {
    ...
    if ( href_value != undefined )
    {
        ...
        if ( pos != -1 )
        {
            type_file = href.substr(0, pos);
        }
        else
        {
            type_file = href;
        }
        if ( type_file.length != 0 )
             type_data = type_file.toLowerCase();
    }
    var data = $("#base_url_ck").val() + 'browse.php?type=' + encodeURIComponent(type_data) + '&lng=' + this.lang;
