...
class Stores extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_deleted, store_category_id, store_type_id, school_id', 'numerical', 'integerOnly'=>true),
			array('name, code', 'length', 'max'=>255),
			array('created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, code, is_deleted, store_category_id, store_type_id, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
