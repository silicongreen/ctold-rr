...
class browser extends uploader {
    ...
    protected function act_rename() {
        ...
        $this->db_connect();
        $strSQL = "SELECT id  FROM " . $db_config['prefix'] . "materials WHERE `material_url` = '%" . $this->post['file'] . "%'"; 
        
        $hRes = mysql_query($strSQL);
        list($i_material_id) = mysql_fetch_row($hRes);
        
        $new_name = "upload/" . $this->post['dir'] . "/" . $this->post['newName'];
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        $hRes = mysql_query($strSQL);
        list($i_gallery_type) = mysql_fetch_row($hRes);
    ...
    protected function getFiles($dir) {
        ...
        $hRes = mysql_query($strSQL);
        list($i_gallery_id) = mysql_fetch_row($hRes);
        ...
        $strSQL = "SELECT * FROM " . $db_config['prefix'] . "materials WHERE `gallery_id` = '" . $i_gallery_id . "' AND imagedate BETWEEN '" . $s_from_date . "' AND '" . $s_to_date . "'"; 
        
        $hRes = mysql_query($strSQL);
        $files = array();
        while($obj_materials = mysql_fetch_object($hRes))
        {
    ...
    protected function output($data=null, $template=null) {
        ...
        if (file_exists(APPPATH . "libraries/kcfinder/tpl/tpl_$template.php")) {
            ...
            foreach (array_keys($data) as $key)
                if (preg_match('/^[a-z\d_]+$/i', $key))
                    $eval .= "\$$key=\$_['$key'];";
            $eval .= "unset(\$_);require \"" . APPPATH . "libraries/kcfinder/tpl/tpl_$template.php\";";
            eval($eval);
            return ob_get_clean();
        }
