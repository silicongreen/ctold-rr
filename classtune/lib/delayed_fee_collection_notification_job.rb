...
class DelayedFeeCollectionNotificationJob
  ...
  def initialize(user,fee_collection_id, batches, one_batch)
    @user = user
  ...
  #          recipient_ids << s.immediate_contact.user_id if s.immediate_contact.present?
        ...
        @batches.each do |b|
          ...
          unless @fee_category.fee_particulars.all(:conditions=>"is_deleted=false and batch_id=#{batch_id}").collect(&:receiver_type).include?"Batch"
            cat_ids=@fee_category.fee_particulars.select{|s| s.receiver_type=="StudentCategory"  and (!s.is_deleted and s.batch_id==batch_id.to_i)}.collect(&:receiver_id)
            student_ids=@fee_category.fee_particulars.select{|s| s.receiver_type=="Student" and (!s.is_deleted and s.batch_id==batch_id.to_i)}.collect(&:receiver_id)
            @students = @students.select{|stu| (cat_ids.include?stu.student_category_id or student_ids.include?stu.id)}
          end
          body = "<p><b>#{t('fee_submission_date_for')} <i>"+fee_category_name.to_s+"</i> #{t('has_been_published')} </b>
            \n \n  #{t('start_date')} : "+@finance_fee_collection.start_date.to_s+" \n"+
