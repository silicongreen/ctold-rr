...
class Student < ActiveRecord::Base
  ...
  named_scope :active, :conditions => { :is_active => true,:is_deleted=>false }
  named_scope :with_full_name_only, :select=>"id,class_roll_no, CONCAT_WS('',first_name,' ',last_name) AS name,first_name,last_name", :order=>:first_name
  ...
  def create_user_and_validate
    unless self.pass.blank?
      if self.pass == "1"
        self.pass = (('0'..'9').to_a + ('a'..'z').to_a + ('A'..'Z').to_a).shuffle.first(6).join
      elsif self.pass == "0"
        self.pass = "123456"
      end
    end
    if self.new_record?
      ...
      user_record.pass = self.pass.blank? ? "123456" : self.pass.to_s
      unless self.save_log.nil?
        if self.save_log
          user_record.save_to_log = self.save_log
        end
      end
      
      unless self.save_to_free.nil?
        if self.save_to_free
          user_record.save_to_free = 1
          user_record.gender = self.gender
          user_record.nationality_id = self.nationality_id
          user_record.birth_date = self.date_of_birth
          user_record.middle_name = self.middle_name
        end
      end
...
#        self.user.password = ("123456") if check_changes.include?('admission_no')
        ...
        unless tds_freeuser.blank?
          tds_freeuser.paid_username = self.user.username
          tds_freeuser.email = self.user.username
          tds_freeuser.first_name = self.first_name
          tds_freeuser.middle_name = self.middle_name
          tds_freeuser.last_name = self.last_name
          tds_freeuser.save
        end
      ...
      if check_changes.include?('immediate_contact_id') or check_changes.include?('admission_no')
        unless ARGV[0].nil? and ARGV[1].nil?
          Guardian.shift_user(self, ARGV[0], ARGV[1])
          ARGV[0] = nil
          ARGV[1] = nil
        else
  ...
  def check_fee_pay(date)
    Champs21Precision.set_and_modify_precision(date.finance_fees.first(:conditions=>"student_id = #{self.id}").balance).to_f == Champs21Precision.set_and_modify_precision(0).to_f
  end
  ...
  def check_dependency
    ...
    return true if Champs21Plugin.check_dependency(self,"permanant").present?
    return false
  end

  def former_dependency
    plugin_dependencies = Champs21Plugin.check_dependency(self,"former")
  end
  ...
  def self.students_details (parameters)
    ...
    if subject_id.nil?
      if sort_order.nil?
        students= Student.all(:select=>"first_name,middle_name,last_name,class_roll_no,admission_no,admission_date,gender,has_paid_fees,CONCAT(courses.code,'-',batches.name) as batch_name, courses.course_name,courses.code,courses.section_name,courses.id as course_id,batches.id as batch_id,students.id as student_id ,count(IF(finance_fees.is_paid=0 and finance_fee_collections.is_deleted=0,1,NULL)) as fee_count",:joins=>"INNER JOIN `batches` ON `batches`.id = `students`.batch_id INNER JOIN `courses` ON `courses`.id = `batches`.course_id LEFT OUTER JOIN finance_fees ON finance_fees.student_id = students.id LEFT OUTER JOIN finance_fee_collections ON finance_fee_collections.id = finance_fees.fee_collection_id",:group=>'students.id',:order=>'first_name ASC')
      else
  ...
  def self.batch_wise_students (parameters)
    ...
    unless config.config_value == 'Daily'
      ...
      unless gender.present?
        if sort_order.nil?
          students= Student.all(:select=>"students.id,admission_date,first_name,middle_name,last_name,class_roll_no,admission_no,gender,(#{academic_days}-count(DISTINCT IF(subject_leaves.month_date>=#{month_date} and subject_leaves.batch_id=#{batch_id},subject_leaves.id,NULL)))/#{academic_days}*100 as percent, count(DISTINCT IF(finance_fees.balance > 0,finance_fees.id,NULL)) as fee_count", :joins=>'LEFT OUTER JOIN subject_leaves ON subject_leaves.student_id=students.id LEFT OUTER JOIN finance_fees ON finance_fees.student_id = students.id',:group=>'students.id',:conditions=>{:batch_id=>batch_id},:order=>'first_name ASC')
        else
  ...
  def self.course_wise_students(parameters)
    ...
    unless gender.present?
      if sort_order.nil?
        students= Student.all(:select=>"first_name,middle_name,last_name,class_roll_no,admission_no,admission_date,gender,has_paid_fees,CONCAT(courses.code,'-',batches.name) as batch_name,count(IF(finance_fees.is_paid=0 and finance_fee_collections.is_deleted=0,1,NULL)) as fee_count",:joins=>"INNER JOIN `batches` ON `batches`.id = `students`.batch_id INNER JOIN `courses` ON `courses`.id = `batches`.course_id LEFT OUTER JOIN finance_fees ON finance_fees.student_id = students.id LEFT OUTER JOIN finance_fee_collections ON finance_fee_collections.id = finance_fees.fee_collection_id",:group=>'students.id',:conditions=>{:courses=>{:id=>course_id}},:order=>'first_name ASC')
      else
