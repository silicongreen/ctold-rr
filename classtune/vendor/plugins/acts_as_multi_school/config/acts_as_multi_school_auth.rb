authorization do
  ...
  role :own_user_profile do
    has_permission_on :admin_users, :to=>[:show,:change_password,:edit,:update]
  end
  ...
  role :multi_school_admin do
    ...
    has_permission_on :admin_users, :to=>[:index,:new,:create, :assign_school, :create_defaults, :defaults_list, :edit_default, :delete_default, :update_default, :defaults_list_data]
    has_permission_on :feeds, :to=>[:index,:new_category,:edit_category,:delete_category,:arrange_category,:save_priorities]
    has_permission_on :package, :to=>[:index,:new_package,:edit_package,:delete_package,:create_package, :package_modules, :edit_package_modules, :edit_package_data]
