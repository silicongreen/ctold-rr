...
class TimetableEntriesController < ApplicationController
  ...
  def update_multiple_timetable_entries2
    ...
    tte_ids.each do |tte_id|
      ...
      start_timing = classtiming.start_time;
      end_timing = classtiming.end_time;
      
      overlapclasstiming = ClassTiming.find(:all,:conditions=>["((start_time < ? and end_time > ?) OR (start_time < ? and end_time > ?) OR (start_time < ? and end_time > ?)) and is_deleted = ?", start_timing,start_timing,end_timing,end_timing,end_timing,start_timing,false]);
    
      
      unless overlapclasstiming.empty?
        overlapclasstiming.each do |classtimings|
          if overlap.empty?
            overlap= TimetableEntry.find(:all,:conditions=>{:employee_id=>emp_ids,:timetable_id=>@timetable.id,:class_timing_id=>classtimings.id,:weekday_id=>weekday},:joins=>"INNER JOIN subjects ON timetable_entries.subject_id = subjects.id INNER JOIN batches ON subjects.batch_id = batches.id AND batches.is_active = 1 AND batches.is_deleted = 0")
          else
            break
          end  
        end
      ...
      unless overlap.empty?
        ...
        overlap.each do |overlap|
          errors["messages"] << "#{t('class_overlap')}: #{overlap.batch.full_name} - #{WeekdaySet.default_weekdays[overlap.weekday_id.to_s]} (#{ClassTiming.find(overlap.class_timing_id).name})."
        end
      else
        tte = TimetableEntry.find_by_weekday_id_and_class_timing_id_and_batch_id_and_timetable_id(weekday,class_timing,@batch.id,@timetable.id)
  ...
  def tt_entry_update2
    ...
    unless overlapclasstiming.empty?
      overlapclasstiming.each do |classtimings|
        if overlapped_tte.nil? or overlapped_tte.empty?
          overlapped_tte= TimetableEntry.find(:all,:conditions=>{:employee_id=>params[:emp_id],:timetable_id=>@timetable.id,:class_timing_id=>classtimings.id,:weekday_id=>weekday},:joins=>"INNER JOIN subjects ON timetable_entries.subject_id = subjects.id INNER JOIN batches ON subjects.batch_id = batches.id AND batches.is_active = 1 AND batches.is_deleted = 0")
        else
          break
        end  
      end
  ...
  def tte_from_batch_and_tt(tt)
    ...
    @ele_subjects = Subject.find_all_by_batch_id(@batch.id, :conditions=>["elective_group_id IS NOT NULL AND is_deleted = false AND elective_groups.batch_id = subjects.batch_id"], :group => "elective_group_id",:include=>[:elective_groups])
  end
