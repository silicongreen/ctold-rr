...
class FreeuserController extends Controller
{
    ...
    {
        $id = Yii::app()->request->getPost('product_id');
        $session = Yii::app()->request->getPost('session');
        $client = new SoapClient('http://182.160.115.227/c-mart/api/soap/?wsdl');
        if (!$session)
        {
            $session = $client->login('meganto', '159753');
        }
        if ($id)
        {
            $product_array = array();
            $result_products = $client->call($session, 'catalog_product.info', $id);
            $product_media = $client->call($session, "catalog_product_attribute_media.list", $id);

            $attribute_set = $client->call($session, "product_attribute.list", array($result_products['set']));

            $product_qty = $client->call($session, 'cataloginventory_stock_item.list', $id);
            //$result = $client->call($session, 'product_custom_option.list', $id);
//        
            $response['data']['session'] = $session;
            $response['data']['current_date'] = date("Y-m-d 00:00:00");
            $response['data']['product'] = $result_products;
            $response['data']['product_media'] = $product_media;
            $response['data']['product_qty'] = $product_qty;
            //$response['data']['custom_option'] = $result;


            $response['status']['code'] = 200;
            $response['status']['msg'] = "PRODUCT_INFO";
        }
        else
        {
    ...
    {
        $id = Yii::app()->request->getPost('category_id');
        $session = Yii::app()->request->getPost('session');
        $client = new SoapClient('http://182.160.115.227/c-mart/api/soap/?wsdl');
        if (!$session)
        {
        ...
        {
            $product_array = array();
            $result_products = $client->call($session, 'catalog_category.assignedProducts', array($id));
            $calls = array();
            $calls2 = array();
            ...
            if (count($result_products) > 0)
            {
                ...
                $j = 0;
                foreach ($result_products as $mp)
                {
                    $calls[$j][] = 'catalog_product.info';
                    $calls[$j][] = $mp['product_id'];


                    $calls2[$j][] = 'catalog_product_attribute_media.list';
                    $calls2[$j][] = $mp['product_id'];
                    $j++;
                }

                $product = $client->multiCall($session, $calls);

                $product_media = $client->multiCall($session, $calls2);

                $k = 0;
                foreach ($product as $key => $pvalue)
                {

                    $product_array['products'][$k]['product_id'] = $pvalue['product_id'];
                    $product_array['products'][$k]['name'] = $pvalue['name'];
                    $product_array['products'][$k]['price'] = $pvalue['price'];
                    $product_array['products'][$k]['special_price'] = $pvalue['special_price'];
                    $product_array['products'][$k]['special_from_date'] = $pvalue['special_from_date'];
                    $product_array['products'][$k]['special_to_date'] = $pvalue['special_to_date'];
                    if (!isset($product_media[$key][0]['url']))
                    {
                        $product_array['products'][$k]['media'] = "";
                    }
                    else
                    {
                        $product_array['products'][$k]['media'] = $product_media[$key][0]['url'];
                    }
            ...
            $response['data']['session'] = $session;
            $response['data']['current_date'] = date("Y-m-d 00:00:00");
            $response['data']['category_products'] = $product_array;
            $response['status']['code'] = 200;
            $response['status']['msg'] = "Product";
        }
        else
        {
    ...
    public function actionProducts()
    {
        ...
        $categoryId = 2; // Put here your category id
        $storeId = 1; // You can add store level
        $result = $client->call($session, 'catalog_category.level', array(1, 1, 2));
        ...
        foreach ($result as $value)
        {
            ...
            $result_products = $client->call($session, 'catalog_category.assignedProducts', array($value['category_id'], null, 3));
            $calls = array();
            $calls2 = array();

            if (count($result_products) > 0)
            {
                $product_array[$i]['category_name'] = $value['name'];
                $product_array[$i]['category_id'] = $value['category_id'];
                ...
                foreach ($product as $key => $pvalue)
                {
                    ...
                    $product_array[$i]['products'][$k]['product_id'] = $pvalue['product_id'];
                    $product_array[$i]['products'][$k]['name'] = $pvalue['name'];
                    $product_array[$i]['products'][$k]['price'] = $pvalue['price'];
                    $product_array[$i]['products'][$k]['special_price'] = $pvalue['special_price'];
                    $product_array[$i]['products'][$k]['special_from_date'] = $pvalue['special_from_date'];
                    $product_array[$i]['products'][$k]['special_to_date'] = $pvalue['special_to_date'];
                    if (!isset($product_media[$key][0]['url']))
                    {
                        $product_array[$i]['products'][$k]['media'] = "";
                    }
                    else
                    {
                        $product_array[$i]['products'][$k]['media'] = $product_media[$key][0]['url'];
                    }
        ...
        $response['data']['session'] = $session;
        $response['data']['current_date'] = date("Y-m-d 00:00:00");
        $response['data']['all_category_products'] = $product_array;
        $response['status']['code'] = 200;
        $response['status']['msg'] = "Product";
        echo CJSON::encode($response);
        Yii::app()->end();
