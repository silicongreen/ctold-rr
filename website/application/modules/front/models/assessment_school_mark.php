...
class Assessment_school_mark extends DataMapper {
    ...
    function get_assesment_school_mark_by_id($id) {
        ...
        $this->db->select('*');
        $this->db->from("assessment_school_mark");
        $this->db->where("assessment_school_mark.id", $id);
    ...
    function find_assessment_school_mark($user_id = 0, $assessment_id = 0, $cur_level = 0, $school_id = 0, $ar_select = array()) {
        ...
        if($user_id > 0) {
            $this->db->where("assessment_school_mark.user_id", $user_id);
        }
        if($assessment_id > 0) {
            $this->db->where("assessment_school_mark.assessment_id", $assessment_id);
        }
        
        if($cur_level > 0) {
            $this->db->where("assessment_school_mark.level", $cur_level);
        }
        
        if($school_id > 0) {
            $this->db->where("assessment_school_mark.school_id", $school_id);
        }
