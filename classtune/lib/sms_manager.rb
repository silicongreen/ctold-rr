...
class SmsManager
  ...
  def perform
    if @config.present?
      power_sms_schools = [2,357,352,361,3,360,312,319,356,362]
      ...
      if power_sms_schools.include?(MultiSchool.current_school.id)
        ...
        parsed_url = @sms_url+"?userId="+@username+"&password="+@password+"&smsText="+encoded_message+"&commaSeperatedReceiverNumbers="+commaseprated
        uri = URI(parsed_url)
