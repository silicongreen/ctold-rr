class MarksController < ApplicationController
  ...
  def get_exams
    ...
    if data_type.to_i == 8
      ...
      @exam_connect = ExamConnect.active.find(:all,:conditions=>"batch_id="+batch_id+" and (result_type=1 or result_type=2 or result_type=5  or result_type=6  or result_type=7  or result_type=8  or result_type=10  or result_type=11) and is_deleted=0")
    else
        ...
        if !@subjects.include?(exam.subject_id) 
          if @employee_subjects.include?(exam.subject_id) or @current_user.admin?
            ...
            if school_id == 340
              ...
              unless c_exam_array.include?(exam_connect.id.to_i)
                data[k] = []
                if (exam_connect.result_type == 13 and school_id != 352)  or (exam_connect.result_type == 14 and school_id != 352)  or (exam_connect.result_type == 15 and school_id != 352)
                  data[k][0] = exam_connect_batch.to_s
