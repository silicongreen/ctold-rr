  ...
  def update_monthly_report_fees
    ...
    if date_format_check
      unless @start_date > @end_date
        ...
        @particular_wise_transactions = FinanceTransactionParticular.find(:all, :select => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.name, finance_fee_particular_categories.id as finance_fee_particular_category_id, sum( finance_transaction_particulars.amount ) as amount", :order => 'finance_transaction_particulars.transaction_date ASC', :conditions => ["finance_transaction_particulars.particular_type = 'Particular' and finance_transaction_particulars.transaction_type = 'Fee Collection' and finance_transaction_particulars.transaction_date >= '#{@start_date}' and finance_transaction_particulars.transaction_date <= '#{@end_date}' and finance_fee_particular_categories.is_deleted = #{false}"], :joins => "INNER JOIN finance_fee_particulars ON finance_fee_particulars.id = finance_transaction_particulars.particular_id INNER JOIN finance_fee_particular_categories ON finance_fee_particular_categories.id = finance_fee_particulars.finance_fee_particular_category_id", :group => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.id")
        @transactions_advances = FinanceTransactionParticular.find(:all, :select => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.name, IFNULL(finance_fee_particular_categories.id, 0) as finance_fee_particular_category_id, sum( finance_transaction_particulars.amount ) as amount", :order => 'finance_transaction_particulars.transaction_date ASC', :conditions => ["finance_transaction_particulars.particular_type = 'Particular' and finance_transaction_particulars.transaction_type = 'Advance' and finance_transaction_particulars.transaction_date >= '#{@start_date}' and finance_transaction_particulars.transaction_date <= '#{@end_date}' and finance_fee_particular_categories.is_deleted = #{false}"], :joins => "LEFT JOIN finance_fee_particulars ON finance_fee_particulars.id = finance_transaction_particulars.particular_id LEFT JOIN finance_fee_particular_categories ON finance_fee_particular_categories.id = finance_fee_particulars.finance_fee_particular_category_id", :group => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.id")
        @transactions_discount = FinanceTransactionParticular.find(:all, :select => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.name, IFNULL(finance_fee_particular_categories.id, 0) as finance_fee_particular_category_id, sum( finance_transaction_particulars.amount ) as amount", :order => 'finance_transaction_particulars.transaction_date ASC', :conditions => ["finance_transaction_particulars.particular_type = 'Adjustment' and finance_transaction_particulars.transaction_type = 'Discount' and finance_transaction_particulars.transaction_date >= '#{@start_date}' and finance_transaction_particulars.transaction_date <= '#{@end_date}' and finance_fee_particular_categories.is_deleted = #{false}"], :joins => "LEFT JOIN finance_fee_particulars ON finance_fee_particulars.id = finance_transaction_particulars.particular_id LEFT JOIN finance_fee_particular_categories ON finance_fee_particular_categories.id = finance_fee_particulars.finance_fee_particular_category_id", :group => "finance_transaction_particulars.transaction_date, finance_fee_particular_categories.id")
        @transactions_fine = FinanceTransactionParticular.find(:all, :select => "finance_transaction_particulars.transaction_date, 'Fine' as name, 0 as finance_fee_particular_category_id, sum( finance_transaction_particulars.amount ) as amount", :order => 'finance_transaction_particulars.transaction_date ASC', :conditions => ["finance_transaction_particulars.particular_type = 'Fine' and finance_transaction_particulars.transaction_date >= '#{@start_date}' and finance_transaction_particulars.transaction_date <= '#{@end_date}'"], :group => "finance_transaction_particulars.transaction_date")
  ...
  def transaction_deletion
    ...
    if advance_fee_collection
      if @fee_collection_advances.particular_id == 0
        ...
        @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@student.batch_id} and finance_fee_particular_category_id = #{@fee_collection_advances.particular_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@student.batch) and (!par.is_deleted ) }
      end
    else
      @fee_particulars = @date.finance_fee_particulars.all(:conditions=>"batch_id=#{@student.batch_id}").select{|par|  (par.receiver.present?) and (par.receiver==@student or par.receiver==@student.student_category or par.receiver==@student.batch) and (!par.is_deleted ) }
    end
