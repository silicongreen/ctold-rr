...
browser.initFolders = function() {
    ...
    $('div.folder > a > span.folder').mousedown(function(e) {
        e.preventDefault();
        if (e.which === 3) {
            _.unselect();
...
browser.buildTree = function(root, path) {
    ...
    {
        var html = '<div class="folder"><span class="brace">&nbsp;</span><span class="folder regular" style="font-size:18px; font-weight: bold;">' + _.htmlData(root.name) + '</span>';
    }
    if (root.dirs) {
...
browser.changeDir = function(dir) {
    if (dir.children('span.folder').hasClass('regular')) {
        ...
        var dir_data = dir.data('path');
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: browser.baseGetData('chDir'),
            data: {dir:dir.data('path'), "range" : $("#range_data").html(), search_text: $("#srch").val()},
            async: false,
            success: function(data) {
                if (browser.check4errors(data))
                    return;
                ...
                browser.statusDir();
                if ( dir_data.indexOf("Gallery/Video") != -1 )
                {
                    $("#upload").hide();
                    $('#toolbar a[href="kcact:upload"]').click(function() {
                        ...
                        var pos = dir_data.lastIndexOf("/");
                        if ( pos != -1 )
                        {
                            var cur_gallery = dir_data.substr( pos + 1, dir_data.length );
                            $("#add_video_frame").attr("src", $("#base_url_ci").val() + "admin/gallery/add_video/" + cur_gallery);
                            var data = $("#video_upload").html();
                            $('#dialog').html(data);
                            $('#dialog').data('title', browser.label("Upload Video"));
                            browser.showDialog();
                        }
                        return false;
                    });
                ...
                {
                    $('#toolbar a[href="kcact:upload"]').unbind("click");
                    $("#upload").show();
                }
            },
            error: function() {
