		...
		_findParticles: function(){
			...
			this.$element.find('[data-stellar-ratio]').each(function(i){
				...
				self.particles.push({
					...
					isHidden: false,
					horizontal: $this.data('stellar-horiz') !== undefined ? 1 : 0,
				});
			});
		},
		...
		_repositionElements: function() {
			...
			for (i = this.particles.length - 1; i >= 0; i--) {
				...
				if (isVisibleHorizontal && isVisibleVertical) {
					...
					if (this.options.horizontalScrolling && particle.horizontal === 1) {
						//console.log('ID: ' + particle.$element.attr('id') + ' / P: ' + particle.horizontal);
						this._setLeft(particle.$element, newPositionLeft, particle.startingPositionLeft);
					}

					if (this.options.verticalScrolling && particle.horizontal === 0) {
