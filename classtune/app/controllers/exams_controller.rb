...
class ExamsController < ApplicationController
  ...
  def show
    ...
    if is_elective == nil
      ...
      assigned_students = StudentsSubject.find_all_by_subject_id_and_batch_id(exam_subject.id,exam_subject.batch_id,:group=>[:student_id])
      @students = []
        ...
        unless student.nil?
          if student.batch_id.to_i == s.batch_id
            if MultiSchool.current_school.id == 319
              ...
              class_roll_no = "1000"
              unless student.class_roll_no.blank?
                class_roll_no = student.class_roll_no
              end
              @students.push [class_roll_no,student.first_name, student.id, student] unless student.nil? 
            elsif !student.class_roll_no.blank? and !student.first_name.blank? and !student.id.blank?
              class_roll_no = 1000
              unless student.class_roll_no.blank?
    ...
    params[:exam].each_pair do |student_id, details|
      ...
      if @exam_score.nil?
        ...
        if details[:marks].to_f <= @exam.maximum_marks.to_f
          ...
          if details[:marks].downcase == "ab" or details[:marks].downcase == "na" or details[:marks].downcase == "n/a" or details[:marks].to_f < 0
            @exam_score.destroy
          else
