    ...
    <div class="post-content clearfix" style="<?php echo ( $news->post_type == 4 || $news->post_type == 3) ? 'background: transparent;' : ''; ?><?php echo ($is_exclusive_found) ? 'height:278px;' : '' ?>">
        <div class="intro-post <?php if($is_exclusive_found===true): ?>col-xs-6<?php endif; ?>">
            <div class="post-entry" > 
                <div class="post-title" style="border-top: 4px solid #DB5257; border-bottom: 4px solid #DB5257;  padding-top:5px; padding-bottom: 5px; margin-top: 120px;">
                    <?php if ($target == "index") : ?>
        ...
        <?php elseif ($news->post_type != 2) : ?>        
            <div class="post-entry <?php if($is_exclusive_found===true): ?>col-xs-6<?php endif; ?>" > 
                ...
                <?php if(count($images)>0): ?>
                    <div class="post-title" <?php echo ($is_exclusive_found===true)? 'style="margin: 24px 0px;"' : 'style="border-top: 3px solid #eee; border-bottom: 3px solid #eee; margin-top: 40px; float: left; width: 100%;"'?>>
                       <?php $l=0;foreach( $images as $image ) :?>
                        <div style="float: left; <?php echo ( $l + 1 < count($images) ) ? 'border-right: 3px solid #eee; height: 300px;' : ''; ?> width: 50%;">
                            <h2 class="f5"><?php echo $image->source; ?></h2>
                            <img width="128"  src="<?php echo base_url($image->image); ?>">
