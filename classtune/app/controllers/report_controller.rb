class ReportController < ApplicationController
  ...
  before_filter :check_permission,:only=>[:index,:course_batch_details,:former_students,:former_employees,:subject_details,:employee_subject_association,:employee_payroll_details,:exam_schedule_details,:fee_collection_details,:course_fee_defaulters,:student_wise_fee_defaulters]
  def index
  ...
  def subject_details
    ...
    if params[:subject_search].nil?
      ...
      if params[:course][:course_id] == "0"                
        ...
        course_id = 0
        params[:course][:course_id] = 0
        unless params[:course_id].nil?
          course_id = params[:course_id]
          params[:course][:course_id] = params[:course_id]
        end
        if course_id == 0
          unless params[:student][:section].nil?
            course_id = params[:student][:section]
            params[:course][:course_id] = params[:student][:section]
          end
        end
        ...
        if course_id.to_i > 0
          ...
          params[:subject_search][:batch_ids] = 0
          unless @batch_data.nil?
            params[:subject_search][:batch_ids] = @batch_data.id 
          end
        else
          params[:subject_search][:batch_ids] = params[:batch_id]
        end      
      end        
