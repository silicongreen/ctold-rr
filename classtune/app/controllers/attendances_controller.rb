...
class AttendancesController < ApplicationController
  ...
  def remove_class_open
    @class_open = ClassOpen.find(params[:id])
    @class_open.destroy
    flash[:notice] = "Successfully Removed"
    redirect_to  :action => "class_open",:page=>params[:page]
  end
  def class_open
    @class_opens = ClassOpen.paginate(:order=>"created_at DESC",:page => params[:page], :per_page => 10)
  end
  def add_class_open
    @batches = Batch.active
    @employee_department = EmployeeDepartment.find(:all, :conditions=>"status = true",:order=>"name ASC")
    @class_open = ClassOpen.new(params[:class_open])
    if request.post? and @class_open.save
      ClassOpenUser.delete_all("class_open_id ="+@class_open.id.to_s)
      reminder_recipient_ids = []
      if @class_open.is_common == 1
        @users = User.active.find(:all)
        reminder_recipient_ids << @users.map(&:id)
      else
        if @class_open.is_common == 0
          batch_id_list = params[:select_options][:batch_id] unless params[:select_options].nil?
          
          unless batch_id_list.nil?
            @batch_students = Student.find_all_by_batch_id(batch_id_list)
            ...
            batch_id_list.each do |c|
              ClassOpenUser.create(:class_open_id => @class_open.id,:batch_id=>c)
            end
          end
          department_id_list = params[:select_options][:department] unless params[:select_options].nil?
          unless department_id_list.nil?
            @dept_emp = Employee.find(:all, :conditions=>["employee_department_id in (?)",department_id_list])
            @dept_emp.each do |e|
              reminder_recipient_ids << e.user_id
            end
            department_id_list.each do |d|
              ClassOpenUser.create(:class_open_id => @class_open.id,:department_id=>d)
            end
          end
        end 
        if @class_open.is_common == 2 or @class_open.is_common == 3
          student_id_list = params[:list][:student_ids] unless params[:list].nil?
          unless student_id_list.nil?
            student_id_list.each do |c|
              reminder_recipient_ids << c
              ClassOpenUser.create(:class_open_id => @class_open.id,:user_id=>c)
            end
          end
        end    
      ...
      unless reminder_recipient_ids.empty?
        Delayed::Job.enqueue(DelayedReminderJob.new( :sender_id  => current_user.id,
            ...
            :subject=>"Class will be open on "+@class_open.date.to_s,
            :rtype=>500,
            :rid=>@class_open.id,
            :body=>"Class will be open on "+@class_open.date.to_s ))
      end
      redirect_to  :action => "class_open"
    end
  end
  def subject_report
    @subjects = []
  ...
  def get_subject_batch_report_excel
    ...
    if params[:batch_id].present?
      ...
      @subject_att_register.each do |val|
        ...
        row_first << ""
        row_first << ""
        new_book.worksheet(0).merge_cells(3,start_row,3,start_row+2)
        start_row = start_row+3
      end 
      ...
      if params[:option].to_i == 1
        if @current_user.employee?
          ...
          unless batch_ids.blank?
            ...
            @students = Student.find(:all, :conditions => "batch_id in (" + batch_ids.join(",") + ")")
            std_subject = StudentsSubject.find(:all, :conditions => "batch_id in (" + batch_ids.join(",") + ")")
  ...
  def index
    ...
    if current_user.admin?
      @batches = Batch.active
    elsif (MultiSchool.current_school.code == "sagc" || MultiSchool.current_school.code == "ess") && @current_user.employee?
