...
class BlogPosts extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_active, is_published, is_deleted, blog_id, school_id', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>255),
			array('body, created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, body, is_active, is_published, is_deleted, blog_id, created_at, updated_at, school_id', 'safe', 'on'=>'search'),
		);
	}
